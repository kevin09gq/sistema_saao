...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Asegurar estado inicial de UI (ocultar ranking en Mixto, etc.)
...
function cambiarTab(tab) {
    ...
    // Ocultar ranking si estamos en extras o mixto
    const btnRanking = document.getElementById('btn_ver_ranking');
    if (btnRanking) {
        btnRanking.style.display = (tab === 'extras' || tab === 'general') ? 'none' : 'block';
    }
...
function pintarDashboard(data) {
    ...
    if (tabActual === 'poda') {
        ...
        kpiDineroTitle.innerText = 'Inversión Total (Podas + Extras)';
    }

    // Mostrar/ocultar/atenuar KPIs según tab
    const kpiArbolesBox = document.getElementById('res_total_arboles').closest('.kpi-box');

    if (tabActual === 'general') {
        // Mixto: mostrar totales consolidados (Podas + Extras)
        kpiArbolesBox.style.opacity = '1';
        kpiDineroBox.style.opacity = '1';
    } else if (tabActual === 'extras') {
...
function abrirDetallesPoda(index, desdeRanking = false) {
    ...
    // En Mixto, si el registro es SOLO extras (sin árboles), el detalle debe verse como "Extras"
    const tieneArboles = parseInt(poda.total_arboles || 0) > 0;
    const tieneExtras = parseFloat(poda.total_extras || 0) > 0;
    let modoDetalle = tabActual;
    if (tabActual === 'general') {
        if (tieneArboles && !tieneExtras) modoDetalle = 'poda';
        else if (!tieneArboles && tieneExtras) modoDetalle = 'extras';
        else modoDetalle = 'general';
    }
    ...
    if (btnPdf) {
        btnPdf.onclick = () => exportarPDFUnicaPoda(index, modoDetalle);
    }

    // Título e ícono dinámicos del modal
    const icon = document.getElementById('det_tipo_icon');
    const text = document.getElementById('det_tipo_text');
    if (icon && text) {
        if (modoDetalle === 'extras') {
            icon.className = 'bi bi-plus-circle-fill me-2';
            text.innerText = 'Detalles de Extras';
        } else if (modoDetalle === 'poda') {
            icon.className = 'bi bi-scissors me-2';
            text.innerText = 'Detalles de Poda';
        } else {
            icon.className = 'bi bi-grid-fill me-2';
            text.innerText = 'Detalles Mixto';
        }
    }
        ...
        .then(res => {
            ...
            if (res.status === 'success' && res.data.length > 0) {
                res.data.forEach(m => {
                    ...
                    if (modoDetalle === 'poda' && m.es_extra == 1) return;
                    if (modoDetalle === 'extras' && m.es_extra == 0) return;
                ...
                const colspan = modoDetalle === 'extras' ? 3 : 5;
...
function exportarExcel() {
    ...
    const tipo = (tabActual === 'general') ? 'mixto' : tabActual;
...
function exportarPDFUnicaPoda(index, modo = tabActual) {
    if (!window.podasDataActual || !window.podasDataActual[index]) return;
    const poda = window.podasDataActual[index];
    ...
    if (!poda.movimientos) {
        ...
        fetch('../php/historial/get_historial_podas.php', {
            ...
            .then(res => {
                ...
                if (res.status === 'success') {
                    poda.movimientos = res.data;
                    generarPDFPodaCorporativo(poda, modo);
                } else {
 ...
 */
function generarPDFPodaCorporativo(poda, modo = tabActual) {
    ...
    // Título dinámico
    const tituloReporte = modo === 'extras' ? 'REPORTE DE PAGOS EXTRAS' :
        modo === 'poda' ? 'REPORTE DE PODAS' : 'REPORTE GENERAL DE PAGOS';
    ...
    doc.text('Rancho Pilar', PAGE_W / 2, y, { align: 'center' });
...
function ordenarRanking(dir) {
    ...
    if (btnDesc && btnAsc) {
        if (dir === 'desc') {
            btnDesc.className = 'btn btn-sm btn-success fw-bold px-3 active';
            btnAsc.className = 'btn btn-sm btn-outline-success fw-bold px-3';
        } else {
            btnAsc.className = 'btn btn-sm btn-success fw-bold px-3 active';
            btnDesc.className = 'btn btn-sm btn-outline-success fw-bold px-3';
        }
    }
...
function renderizarTablaRanking() {
    ...
    if (!tbodyModalRanking) return;
    tbodyModalRanking.innerHTML = '';

    if (!window.rankingPodadoresActual) return;

    // Por si llegan duplicados (mismo empleado en múltiples registros), consolidar.
    const mapa = new Map();
    window.rankingPodadoresActual.forEach(r => {
        const nombre = (r.nombre_empleado || '').trim();
        if (!nombre) return;
        const arboles = parseInt(r.total_arboles || 0);
        if (!mapa.has(nombre)) {
            mapa.set(nombre, {
                nombre_empleado: nombre,
                total_arboles: arboles,
                id_poda: r.id_poda
            });
        } else {
            const actual = mapa.get(nombre);
            actual.total_arboles += arboles;
            // Conservar algún id_poda válido para abrir detalles
            if (!actual.id_poda && r.id_poda) actual.id_poda = r.id_poda;
        }
    });

    let lista = Array.from(mapa.values()).filter(r => parseInt(r.total_arboles) > 0);
    ...
    lista.forEach((r, idx) => {
        ...
        let originalIndex = -1;
        if (window.podasDataActual && Array.isArray(window.podasDataActual)) {
            if (r.id_poda) {
                originalIndex = window.podasDataActual.findIndex(p => p.id_poda == r.id_poda);
            }
            if (originalIndex === -1) {
                originalIndex = window.podasDataActual.findIndex(p => (p.nombre_empleado || '').trim() === r.nombre_empleado);
            }
        }
...
function abrirDetallesDesdeRanking(index) {
    ...
    if (modalRanking) modalRanking.hide();
