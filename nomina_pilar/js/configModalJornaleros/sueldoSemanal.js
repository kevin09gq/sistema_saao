    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.data && response.data.horario_jornalero) {
                ...
                jsonNominaPilar.horarioRancho = horarioRancho;
...
function calcularSueldoSemanal(empleado = null) {
    // Verificar que existe jsonNominaPilar
...
// usa jsonNominaPilar.horarioRancho que es un arreglo de objetos {dia,entrada,salida,...}
function calcularTardeadas(empleado) {
    if (!empleado.registros || !jsonNominaPilar.horarioRancho) return 0;
    ...
    Object.keys(regs).forEach(fecha => {
        ...
        const horarioDia = jsonNominaPilar.horarioRancho.find(h => h.dia === dia);
