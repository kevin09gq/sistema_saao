...
function asignarValoresConfig(statusRancho) {
    $("#btn_config_avanzar_pilar").click(function (e) {
        ...
        let pasajeVal = $("#precio_pasaje_pilar").val().trim();
        let tardeadaVal = $("#pago_tardeada_pilar").val().trim();
        let comidaVal = $("#pago_comida_pilar").val().trim();
...
function quitarTarjeta() {
    ...
    $(document).on('click', '#btn_delete_tarjeta', function () {
        if (typeof jsonNominaPilar === 'undefined' || !jsonNominaPilar || !Array.isArray(jsonNominaPilar.departamentos)) {
