...
function obtenerTablasRancho() {
    // Obtener el id_area del Pilar (siempre es 2)
    const id_area = 2;
...
function llenar_tabla_tickets_pendientes() {
    ...
    // VALIDAR QUE EXISTE EL jsonNominaPilar
    if (!jsonNominaPilar || !jsonNominaPilar.departamentos) {
        console.error("jsonNominaPilar no está definido o no tiene departamentos");
        return;
    }
    ...
    cortesPagina.forEach((corte, index) => {
        // Buscar si el folio ya está en la estructura jsonNominaPilar
        let existeFolio = false;

        // Buscar el departamento Corte
        let departamentoCorte = jsonNominaPilar.departamentos.find(
            d => d.nombre === "Corte"
        );
...
$(document).on("change", ".check_select_corte", function (e) {
    ...
    if (!departamentoCorte) {
        ...
        jsonNominaPilar.departamentos.push(departamentoCorte);
    }
