...
$(document).on('click', '#context_menu_poda', function (e) {
    ...
    switch (concepto) {
        case 'PODA':
            abrir_modal_poda(jsonNominaPilar, nombre, monto);
            break;
        default:
            abrir_modal_poda_extra(jsonNominaPilar, concepto, nombre, monto);
            break;
    }
});
...
$(document).on('input', '#movimientos_poda_body .arboles, #movimientos_poda_body .monto_poda', function (e) {
    ...
    const departamento = jsonNominaPilar.departamentos.find(
        d => d.nombre === "Poda"
    );
