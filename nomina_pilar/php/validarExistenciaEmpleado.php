    ...
    case 'POST':
        ...
        if ($_POST['case'] === 'validarExistenciaTrabajador') {
            if (!isset($_POST['claves']) || !is_array($_POST['claves'])) {
                echo json_encode(['error' => 'No se recibieron claves', 'existentes' => []]);
                exit;
            }
        ...
        break;
    case 'GET':
        if (isset($_GET['case']) && $_GET['case'] === 'obtenerEmpleadosSinSeguroPilar') {
            obtenerEmpleadosSinSeguroPilar();
        } else if (isset($_GET['case']) && $_GET['case'] === 'obtenerEmpleadosSinSeguro') {
...
function validarExistenciaTrabajador(){
    ...
    // Filtrar por cláves Y que el empleado pertenezca al área y departamentos de la nómina 5
    $sql = "SELECT ie.id_empleado, ie.clave_empleado, ie.nombre, ie.ap_paterno, ie.ap_materno, ie.biometrico, ie.id_empresa, ie.id_departamento, ie.id_puestoEspecial 
            ...
            AND ie.id_status = 1
            AND ie.id_empresa = 1
            AND ie.id_area = (SELECT n.id_area FROM nombre_nominas n WHERE n.id_nomina = 5)
            AND ie.id_departamento IN (SELECT nd.id_departamento FROM nomina_departamento nd WHERE nd.id_nomina = 5)";
...
// PASO 2: OBTENER EMPLEADOS DE LOS DEPARTAMENTOS QUE ESTAN RELACIONADO A LA NOMINA PILAR (SIN SEGURO) - VERSION ORIGINAL
function obtenerEmpleadosSinSeguro()
{
    ...
    // Obtener empleados sin seguro que pertenecen al área y departamentos de la nómina 5
    $sql = "SELECT ie.id_empleado, ie.clave_empleado, ie.nombre, ie.ap_paterno, ie.ap_materno, ie.biometrico, ie.id_empresa, ie.id_departamento, ie.id_puestoEspecial
            FROM info_empleados ie
...
// PASO 3: OBTENER LOS DATOS DE SALARIOS Y HORARIOS DE LOS EMPLEADOS 
...
function validarEmpleadosSinSeguroBiometrico()
{
    ...
    // Obtener empleados sin seguro (biométrico) del área y departamentos de la nómina 5
    $sql = "SELECT ie.id_empleado, ie.clave_empleado, ie.nombre, ie.ap_paterno, ie.ap_materno, ie.biometrico,  ie.id_empresa, ie.id_departamento, ie.id_puestoEspecial
            FROM info_empleados ie
            WHERE ie.id_status = 1
            AND ie.id_empresa = 1
...
// PASO 2 VALIDAR EMPLEADOS NUEVOS QUE VENGAN EN LA LISTA DE RAYA
function validarEmpleadosNuevos()
{
    ...
    // Consultar empleados nuevos filtrando por claves, área y departamentos de la nómina 5
    $sql = "SELECT ie.id_empleado, ie.clave_empleado, ie.nombre, ie.ap_paterno, ie.ap_materno, ie.biometrico, ie.salario_semanal, ie.salario_diario,
            ie.id_empresa, ie.id_departamento, ie.id_puestoEspecial, ho.horario_oficial 
            FROM info_empleados ie LEFT JOIN horarios_oficiales ho ON ie.id_empleado = ho.id_empleado 
            WHERE ie.clave_empleado IN ($clavesString)
            AND ie.id_status = 1
            AND ie.id_empresa = 1
            AND ie.id_area = (SELECT n.id_area FROM nombre_nominas n WHERE n.id_nomina = 5)
...
// PASO 3: OBTENER EMPLEADOS SIN SEGURO 
function obtenerEmpleadosSinSeguroPilar()
{
    global $conexion;
