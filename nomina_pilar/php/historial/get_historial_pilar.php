...
try {
    if ($action === 'get_filtros_iniciales') {
        $query = "
            SELECT DISTINCT n.id_nomina_pilar, n.anio, n.numero_semana, n.nomina_pilar
            FROM nomina_pilar n
            JOIN cortes_pilar c ON n.id_nomina_pilar = c.id_nomina
        ...
        while($row = $result->fetch_assoc()) {
            ...
            $mes = extraerMesDesdeJson($row['nomina_pilar']);
    ...
    if ($action === 'buscar') {
        ...
        // Solo filtramos en SQL por año y semana porque esos si los tiene nomina_pilar como columnas
        $params_types = "";
        $params_values = [];
        $where_clauses = ["1=1"];
        ...
        // Obtener cortes principales unidos con nomina_pilar
        $query_cortes = "
            SELECT 
                c.id, c.folio, c.nombre_cortador, c.fecha_corte, c.precio_reja,
                n.numero_semana as sem_corte, n.anio as anio_corte, n.nomina_pilar,
            ...
            FROM cortes_pilar c
            JOIN nomina_pilar n ON c.id_nomina = n.id_nomina_pilar
            LEFT JOIN cortes_pilar_tablas t ON c.id = t.id_corte
            WHERE $where
            GROUP BY c.id
        ...
        while ($c = $result_cortes->fetch_assoc()) {
            ...
            $mes_corte = extraerMesDesdeJson($c['nomina_pilar']);
            
            if ($filtro_mes !== '' && (int)$mes_corte !== (int)$filtro_mes) {
                // Saltar este corte porque no pertenece al mes seleccionado
                continue;
            }
            
            // Si pasa el filtro, descartamos el JSON gigante para no saturar la respuesta de la API
            unset($c['nomina_pilar']);
        ...
        // Obtener el Ranking completo de tablas (solo de los cortes incluidos mediante PHP_filters)
        $ranking = [];
        $mejor_tabla = null;
