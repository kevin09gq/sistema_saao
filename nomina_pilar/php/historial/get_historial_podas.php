...
try {
    if ($action === 'get_filtros_iniciales') {
        $query = "
            ...
            FROM nomina_pilar n
            JOIN podas_pilar p ON n.id_nomina_pilar = p.id_nomina
    ...
    if ($action === 'buscar') {
        ...
        $query_podas = "
            SELECT 
                ...
                n.numero_semana as sem_poda, n.anio as anio_poda, n.nomina_pilar,
        ...
        while ($p = $result_podas->fetch_assoc()) {
            $mes_poda = extraerMesDesdeJson($p['nomina_pilar']);
            if ($filtro_mes !== '' && (int)$mes_poda !== (int)$filtro_mes) {
                continue;
            }

            unset($p['nomina_pilar']);
