...
function cargarDepartamentosExportar() {
    ...
    if (!jsonHistorialPilar || !Array.isArray(jsonHistorialPilar.departamentos)) {
        container.html('<div class="alert alert-warning small">No hay departamentos disponibles</div>');
        return;
    }

    jsonHistorialPilar.departamentos.forEach(depto => {
...
function exportarNominaDepartamento() {
    $(document).on('click', '#btn-export-corte', function (e) {
        ...
        // Validar que jsonHistorialPilar exista
        if (!jsonHistorialPilar) {
            alert('No hay datos de nómina para exportar. Por favor, procesa los datos primero.');
            return;
        }

        // Validar que el departamento exista
        const departamento = jsonHistorialPilar.departamentos.find(d => d.id_departamento == deptoId);
        ...
        // Enviar el jsonHistorialPilar al servidor PHP mediante POST
        $.ajax({
            url: tmp_url,
            type: 'POST',
            data: {
                jsonNomina: JSON.stringify(jsonHistorialPilar),
                deptoId: deptoId,
            ...
            success: function (blob) {
                ...
                link.href = url;
                var numeroSemana = String(jsonHistorialPilar.numero_semana).padStart(2, '0');
                var aniosCierre = jsonHistorialPilar.fecha_cierre.split('/')[2];
...
function reporteNominaPdf() {
    $("#btn_export_pdf_reporte").click(function (e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                numero_semana: jsonHistorialPilar.numero_semana || '',
                fecha_cierre: jsonHistorialPilar.fecha_cierre || '',
                fecha_inicio: jsonHistorialPilar.fecha_inicio || '',
                periodo_nomina: jsonHistorialPilar.periodo_nomina || '',
                jsonNomina: JSON.stringify(jsonHistorialPilar)
            },
            xhrFields: {
                responseType: 'blob'
    ...
    $("#btn-export-dispersion-tarjeta").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                link.href = url;
                var numeroSemana = String(jsonHistorialPilar.numero_semana).padStart(2, '0');
                var anio = jsonHistorialPilar.fecha_cierre.split('/')[2];
...
function validarEmpleadosNegativos() {
    ...
    if (!jsonHistorialPilar || !jsonHistorialPilar.departamentos) {
        return false;
    }
