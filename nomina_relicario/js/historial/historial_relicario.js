...
function cargarFiltrosIniciales() {
    ...
    fetch('../php/historial/get_historial_relicario.php', {
        method: 'POST',
        body: formData
    })
        .then(r => r.json())
...
function generarPDFConLogo(logoBase64, datosExportar, folioUnico = null) {
    ...
    for (let pg = 1; pg <= totalPages; pg++) {
        ...
        doc.text(`Nomina Relicario • Historial de Cortes`, MARGIN, PAGE_H - 6);
        doc.text(`Página ${pg} de ${totalPages}`, PAGE_W - MARGIN, PAGE_H - 6, { align: 'right' });
    }

    const fileLabel = folioUnico ? `Historial_Corte_Relicario_Folio_${folioUnico}.pdf` : `Historial_Cortes_Relicario_${anio}_${mesLabel}.pdf`.replace(/\s/g, '_');
    doc.save(fileLabel);
}
