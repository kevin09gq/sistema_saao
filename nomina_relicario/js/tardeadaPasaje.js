...
function actualizarPasajeTardeadaEnEmpleados() {
    ...
    // Recolectar empleados del departamento 7 que tengan registros
    ...
    if (Array.isArray(jsonNominaRelicario.departamentos)) {
        jsonNominaRelicario.departamentos.forEach(departamento => {
            ...
            departamento.empleados.forEach(empleado => {
                // Solo procesar departamento 7 con registros
                if (parseInt(empleado.id_departamento) === 7 && Array.isArray(empleado.registros)) {
...
function listarEmpleadosParaQuitarComidaPasaje() {
    ...
    // Iterar y filtrar jornaleros (departamento 7 con mostrar = true)
    jsonNominaRelicario.departamentos.forEach(departamento => {
        if (!Array.isArray(departamento.empleados)) return;

        departamento.empleados.forEach(empleado => {
            if (parseInt(empleado.id_departamento) === 7 && empleado.mostrar !== false) {
                const fila = `
                    <tr>
