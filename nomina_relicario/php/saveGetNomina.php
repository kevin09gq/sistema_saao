...
switch ($case) {
    case 'guardarNominaRelicario':
        guardarNominaRelicario($data, $conexion);
        break;
    ...
    if ($result->num_rows > 0) {
        if ($actualizar) {
            ...
            if ($updateStmt->execute()) {
                ...
                // Id de la nomina
                $idNominaActualizada = $rowId['id_nomina_relicario'];
...
/**
 * Función para obtener la nómina del relicario considerando número de semana y año
 * @param Array $data Los datos enviados desde el cliente (debe incluir número_semana, anio e id_empresa)
 * @param mysqli $conexion La conexión a la base de datos
 * @return void Responde con un JSON que indica si se encontró la nómina y su contenido
 */
function obtenerNomina($data, $conexion)
{
    ...
    // Usar la tabla `nomina_relicario` que contiene la columna `anio` según tu esquema
    $anio = isset($data['anio']) ? intval($data['anio']) : 0;
    $query = "SELECT id_nomina_relicario, nomina_relicario FROM nomina_relicario WHERE id_empresa = ? AND numero_semana = ? AND anio = ? ORDER BY id_nomina_relicario DESC LIMIT 1";
    $stmt = $conexion->prepare($query);
    if (!$stmt) {
    ...
    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $nomina_raw = $row['nomina_relicario'];
        $idNomina = $row['id_nomina_relicario'];
        ...
        foreach ($empleadosCorte as $empleado) {
            ...
            if ($empleado['concepto'] === 'REJA' && isset($empleado['tickets']) && is_array($empleado['tickets'])) {
                foreach ($empleado['tickets'] as $ticket) {
                    ...
                    $checkQuery = "SELECT id, id_nomina FROM cortes_relicario WHERE folio = ? AND nombre_cortador = ?";
                    ...
                    if ($row = $result->fetch_assoc()) {
                        ...
                        if (is_null($idNominaExistente)) {
                            ...
                            $updateQuery = "UPDATE cortes_relicario 
                            ...
                            $conexion->query("DELETE FROM cortes_relicario_tablas WHERE id_corte = $idCorte");
                            foreach ($ticket['datosRejas'] as $datosTabla) {
                                $numTabla = intval($datosTabla['tabla']);
                                $cantidadRejas = intval($datosTabla['cantidad']);
                                $insertTablaQuery = "INSERT INTO cortes_relicario_tablas (id_corte, num_tabla, rejas) VALUES (?, ?, ?)";
...
function guardarPoda($poda, $idNomina, $conexion)
{
    try {
         ...
         * ==========================================================
         */
        $deleteQuery = "DELETE FROM podas_relicario WHERE id_nomina = ?";
        $deleteStmt = $conexion->prepare($deleteQuery);
        $deleteStmt->bind_param("i", $idNomina);
        ...
        foreach ($poda as $empleado) {
            ...
            $insertPodaQuery = "INSERT INTO podas_relicario 
            ...
            foreach ($empleado['movimientos'] as $mov) {
                ...
                $insertMovQuery = "INSERT INTO podas_movimientos_relicario 
...
function obtenerPodaInterno($idNomina, $conexion)
{
    ...
    $query = "SELECT 
        ...
        INNER JOIN podas_movimientos_relicario m 
