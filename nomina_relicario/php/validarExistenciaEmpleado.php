...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'POST':
        ...
        break;
    case 'GET':
        if (isset($_GET['case']) && $_GET['case'] === 'obtenerEmpleadosSinSeguroRelicario') {
            obtenerEmpleadosSinSeguroRelicario();
...
function validarExistenciaTrabajador(){
    ...
    // Filtrar por cláves Y que el empleado pertenezca al área y departamentos de la nómina 4
    $sql = "SELECT ie.id_empleado, ie.clave_empleado, ie.nombre, ie.ap_paterno, ie.ap_materno, ie.biometrico, ie.id_empresa, ie.id_departamento, ie.id_puestoEspecial 
            ...
            AND ie.id_status = 1
            AND ie.id_empresa = 1
            AND ie.id_area = (SELECT n.id_area FROM nombre_nominas n WHERE n.id_nomina = 4)
            AND ie.id_departamento IN (SELECT nd.id_departamento FROM nomina_departamento nd WHERE nd.id_nomina = 4)";
...
function validarEmpleadosSinSeguroBiometrico()
{
    ...
    // Obtener empleados sin seguro (biométrico) del área y departamentos de la nómina 4
    $sql = "SELECT ie.id_empleado, ie.clave_empleado, ie.nombre, ie.ap_paterno, ie.ap_materno, ie.biometrico,  ie.id_empresa, ie.id_departamento, ie.id_puestoEspecial
            FROM info_empleados ie
            WHERE ie.id_status = 1
            AND ie.id_empresa = 1
...
function validarEmpleadosNuevos()
{
    ...
    // Consultar empleados nuevos filtrando por claves, área y departamentos de la nómina 4
...
function obtenerEmpleadosSinSeguroRelicario()
{
    global $conexion;
