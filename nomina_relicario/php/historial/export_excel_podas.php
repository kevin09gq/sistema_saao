...
function prepararHoja(
    ...
    $sheet->setCellValue('A2', 'RANCHO RELICARIO - HISTORIAL DE PODAS');
    ...
    // Títulos corporativos más grandes para impresión (Relicario usa rojo)
...
try {
    ...
    $queryNominas = "
        SELECT DISTINCT n.id_nomina_relicario, n.numero_semana, n.anio, n.nomina_relicario
        FROM nomina_relicario n
        JOIN podas_relicario p ON p.id_nomina = n.id_nomina_relicario
        WHERE {$where}
    ...
    while ($row = $resNom->fetch_assoc()) {
        $mesNom = extraerMesDesdeJson($row['nomina_relicario']);
        if ($mesFiltroInt > 0 && (int)$mesNom !== $mesFiltroInt) {
            continue;
        }
        $nominaIds[] = (int)$row['id_nomina_relicario'];
    ...
    $queryMov = "
        ...
        JOIN podas_relicario p ON m.id_poda = p.id_poda
        JOIN nomina_relicario n ON p.id_nomina = n.id_nomina_relicario
        WHERE n.id_nomina_relicario IN ({$placeholders})
    ...
    $spreadsheet->getProperties()
        ...
        ->setTitle('Historial Podas/Extras - Relicario');
    ...
    $filename = "relicario_{$anio}_{$semPart}_{$tipo}.xlsx";
