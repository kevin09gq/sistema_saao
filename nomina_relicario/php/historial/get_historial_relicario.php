...
try {
    if ($action === 'get_filtros_iniciales') {
        $query = "
            ...
            JOIN cortes_relicario c ON n.id_nomina_relicario = c.id_nomina
    ...
    if ($action === 'buscar') {
        ...
        // Solo filtramos en SQL por año y semana porque esos si los tiene nomina_relicario como columnas
        $params_types = "";
        $params_values = [];
        $where_clauses = ["1=1"];
        ...
        // Obtener cortes principales unidos con nomina_relicario
        $query_cortes = "
            SELECT 
                c.id, c.folio, c.nombre_cortador, c.fecha_corte, c.precio_reja,
                n.numero_semana as sem_corte, n.anio as anio_corte, n.nomina_relicario,
            ...
            FROM cortes_relicario c
            JOIN nomina_relicario n ON c.id_nomina = n.id_nomina_relicario
            LEFT JOIN cortes_relicario_tablas t ON c.id = t.id_corte
            WHERE $where
            GROUP BY c.id
        ...
        while ($c = $result_cortes->fetch_assoc()) {
            ...
            $mes_corte = extraerMesDesdeJson($c['nomina_relicario']);
        ...
        if (count($ids_cortes_incluidos) > 0) {
            ...
            $query_ranking = "
                ...
                FROM cortes_relicario_tablas t
