...
function cargarDepartamentosExportar() {
    ...
    if (!jsonHistorialRelicario || !Array.isArray(jsonHistorialRelicario.departamentos)) {
        container.html('<div class="alert alert-warning small">No hay departamentos disponibles</div>');
        return;
    }

    jsonHistorialRelicario.departamentos.forEach(depto => {
...
function exportarNominaDepartamento() {
    $(document).on('click', '#btn-export-corte', function (e) {
        ...
        // Validar que jsonHistorialRelicario exista
        if (!jsonHistorialRelicario) {
            alert('No hay datos de nómina para exportar. Por favor, procesa los datos primero.');
            return;
        }

        // Validar que el departamento exista
        const departamento = jsonHistorialRelicario.departamentos.find(d => d.id_departamento == deptoId);
        ...
        // Enviar el jsonHistorialRelicario al servidor PHP mediante POST
        $.ajax({
            url: tmp_url,
            type: 'POST',
            data: {
                jsonNomina: JSON.stringify(jsonHistorialRelicario),
                deptoId: deptoId,
            ...
            success: function (blob) {
                ...
                link.href = url;
                var numeroSemana = String(jsonHistorialRelicario.numero_semana).padStart(2, '0');
                var aniosCierre = jsonHistorialRelicario.fecha_cierre.split('/')[2];
...
function reporteNominaPdf() {
    $("#btn_export_pdf_reporte").click(function (e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                numero_semana: jsonHistorialRelicario.numero_semana || '',
                fecha_cierre: jsonHistorialRelicario.fecha_cierre || '',
                fecha_inicio: jsonHistorialRelicario.fecha_inicio || '',
                periodo_nomina: jsonHistorialRelicario.periodo_nomina || '',
                jsonNomina: JSON.stringify(jsonHistorialRelicario)
            },
            xhrFields: {
                responseType: 'blob'
    ...
    $("#btn-export-dispersion-tarjeta").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                link.href = url;
                var numeroSemana = String(jsonHistorialRelicario.numero_semana).padStart(2, '0');
                var anio = jsonHistorialRelicario.fecha_cierre.split('/')[2];
...
function validarEmpleadosNegativos() {
    ...
    if (!jsonHistorialRelicario || !jsonHistorialRelicario.departamentos) {
        return false;
    }
