...
function sumasAutomaticasPercepciones() {
    ...
    $(document).on('input', '#mod-horas-extras-10lbs, #mod-bono-antiguedad-10lbs, #mod-actividades-especiales-10lbs, #mod-puesto-10lbs', function () {
        calcularTotalPercepcionesEnTiempoReal();
    });
}
...
function calcularTotalPercepcionesEnTiempoReal() {
    ...
    let totalExtras = 0;
    $('#contenedor-conceptos-adicionales-10lbs').find('.cantidad-percepcion').each(function () {
        const cantidad = parseFloat($(this).val()) || 0;
        totalExtras += cantidad;
    });

    // Sumar horas extras
    const horasExtras = parseFloat($('#mod-horas-extras-10lbs').val()) || 0;
    totalExtras += horasExtras;

    // Sumar bono de antiguedad
    const bonoAntiguedad = parseFloat($('#mod-bono-antiguedad-10lbs').val()) || 0;
    totalExtras += bonoAntiguedad;

    // Sumar actividades especiales
    const actividadesEspeciales = parseFloat($('#mod-actividades-especiales-10lbs').val()) || 0;
    totalExtras += actividadesEspeciales;

    // Sumar puesto
    const puesto = parseFloat($('#mod-puesto-10lbs').val()) || 0;
    totalExtras += puesto;

    // Establecer el total en el campo
    $('#mod-total-extra-10lbs').val(totalExtras.toFixed(2));
...
function calcularTotalDeduccionesEnTiempoReal() {
    ...
    $('#contenedor-deducciones-adicionales-10lbs').find('.cantidad-deduccion').each(function () {
        const cantidad = parseFloat($(this).val()) || 0;
        totalDeducciones += cantidad;
    });

    $('#mod-fagafetcofia-10lbs').val(totalDeducciones.toFixed(2));
    calcularSueldoACobrar();
}
...
function sumasAutomaticasConceptos() {
    ...
    $(document).on('input', '#mod-isr-10lbs, #mod-imss-10lbs, #mod-infonavit-10lbs, #mod-ajustes-sub-10lbs', function () {
...
function actualizarConceptos() {
    $(document).on('click', '#btn-aplicar-isr-10lbs', function () {
        ...
        $('#mod-isr-10lbs').val(copia.resultado);
        calcularTotalConceptosEnTiempoReal();
    });

    $(document).on('click', '#btn-aplicar-imss-10lbs', function () {
        const empleado = objEmpleado.getEmpleado();
        ...
        $('#mod-imss-10lbs').val(copia.resultado);
        calcularTotalConceptosEnTiempoReal();
    });

    $(document).on('click', '#btn-aplicar-infonavit-10lbs', function () {
        const empleado = objEmpleado.getEmpleado();
        ...
        $('#mod-infonavit-10lbs').val(copia.resultado);
        calcularTotalConceptosEnTiempoReal();
    });

    $(document).on('click', '#btn-aplicar-ajuste-sub-10lbs', function () {
        const empleado = objEmpleado.getEmpleado();
        ...
        $('#mod-ajustes-sub-10lbs').val(copia.resultado);
        calcularTotalConceptosEnTiempoReal();
    });


    $(document).on('click', '#btn-aplicar-tarjeta-10lbs', function () {
        const empleado = objEmpleado.getEmpleado();
        ...
        $('#mod-tarjeta-10lbs').val(copiaTarjeta);
        calcularSueldoACobrar();
    });

    //  Al presionar el botón de incentivo, ponerlo en 0
    $(document).on('click', '#btn-aplicar-incentivo-10lbs', function () {
        const empleado = objEmpleado.getEmpleado();
        if (!empleado) return;

        $('#mod-incentivo-10lbs').val('0.00');
...
function validarConceptoMaxTarjeta() {
    $(document).on('input', '#mod-tarjeta-10lbs', function () {
        const empleado = objEmpleado.getEmpleado();
...
function aplicarTotalHistorial() {
    ...
    $(document).on('click', '#btn-aplicar-checador-10lbs', function () {
        const empleado = objEmpleado.getEmpleado();
        if (!empleado) return;
        ...
        $('#mod-checador-10lbs').val(totalChecador.toFixed(2));
    ...
    $(document).on('click', '#btn-calcular-inasistencias-10lbs', function () {
        const empleado = objEmpleado.getEmpleado();
        if (!empleado) return;
        ...
        $('#mod-inasistencias-10lbs').val(totalInasistencias.toFixed(2));
...
function inicializarSueldoACobrar() {
    ...
    $(document).on('input', '#mod-sueldo-neto-10lbs, #mod-total-extra-10lbs', calcularSueldoACobrar);

    // Escuchar cambios en todas las deducciones editables
    $(document).on('input',
        '#mod-tarjeta-10lbs, #mod-prestamo-10lbs, #mod-checador-10lbs, ' +
        '#mod-inasistencias-10lbs, ' +
        '#mod-permisos-10lbs, #mod-fagafetcofia-10lbs',
        calcularSueldoACobrar
    );


    // Mostrar/ocultar opciones de redondeo y recalcular al activar el checkbox
    $(document).on('change', '#mod-redondear-sueldo-10lbs', function () {
    
        calcularSueldoACobrar();
    });

    // Recalcular al cambiar el modo de redondeo
    $(document).on('change', '#mod-redondeo-modo-10lbs', calcularSueldoACobrar);
}
...
function calcularSueldoACobrar() {
    ...
    // ---- PERCEPCIONES ----
    const sueldoNeto = parseFloat($('#mod-sueldo-neto-10lbs').val()) || 0;
    const sueldoExtra = parseFloat($('#mod-total-extra-10lbs').val()) || 0;
    const totalPercepciones = sueldoNeto + sueldoExtra;

    // ---- CONCEPTOS (ISR, IMSS, INFONAVIT, AJUSTE AL SUB) ----
    const totalConceptos = parseFloat($('#mod-total-conceptos-10lbs').val()) || 0;

    // ---- DEDUCCIONES ----
    const tarjeta = parseFloat($('#mod-tarjeta-10lbs').val()) || 0;
    const prestamo = parseFloat($('#mod-prestamo-10lbs').val()) || 0;
    const checador = parseFloat($('#mod-checador-10lbs').val()) || 0;
    const inasistencias = parseFloat($('#mod-inasistencias-10lbs').val()) || 0;
    const permisos = parseFloat($('#mod-permisos-10lbs').val()) || 0;
    const uniforme = parseFloat($('#mod-uniforme-10lbs').val()) || 0;
    const fagafetcofia = parseFloat($('#mod-fagafetcofia-10lbs').val()) || 0;
...
function aplicarRedondeo(totalSinRedondear) {
    const redondeoActivo = $('#mod-redondear-sueldo-10lbs').is(':checked');
    ...
    $('#mod-sueldo-a-cobrar-10lbs').val(totalFinal.toFixed(2));
...
function limpiarModal() {
    ...
    // ========================================
    $('#campo-clave-10lbs').text('');
    $('#campo-nombre-10lbs').text('');
    $('#campo-departamento-10lbs').text('');
    $('#campo-puesto-10lbs').text('');
    $('#campo-id-empresa-10lbs').val('');
    ...
    // En modal10lbs.php no veo estos inputs de "copia rapida", pero los dejo por si acaso o los quito si no aplican.
    // Viendo el modal, no están. Los comentaré o quitaré.

    // ========================================
    // LIMPIAR EVENTOS ESPECIALES
    // ========================================
    $('#entradas-tempranas-10lbs').empty();
    $('#salidas-tardias-10lbs').empty();
    $('#salidas-tempranas-10lbs').empty();
    $('#olvidos-checador-10lbs').empty();
    $('#retardos-10lbs').empty();
    $('#inasistencias-content-10lbs').empty();
    // $('#analisis-permisos-comida-content-10lbs').empty(); // Comentado en el modal

    // Limpiar totales de eventos
    $('#total-entradas-tempranas-10lbs').text('0');
    $('#total-salidas-tardias-10lbs').text('0');
    $('#total-salidas-tempranas-10lbs').text('0');
    $('#total-olvidos-checador-10lbs').text('0');
    $('#total-retardos-10lbs').text('0');
    $('#total-inasistencias-10lbs').text('0');

    // ========================================
    // LIMPIAR TAB: MODIFICAR DETALLES (Percepciones)
    // ========================================
    $('#mod-sueldo-neto-10lbs').val('');
    $('#mod-total-extra-10lbs').val('');
    
    // Limpiar contenedor de conceptos adicionales
    $('#contenedor-conceptos-adicionales-10lbs').empty();
    ...
    $('#mod-isr-10lbs').val('');
    $('#mod-imss-10lbs').val('');
    $('#mod-infonavit-10lbs').val('');
    $('#mod-ajustes-sub-10lbs').val('');
    $('#mod-total-conceptos-10lbs').val('');

    // ========================================
    // LIMPIAR TAB: MODIFICAR DETALLES (Deducciones)
    // ========================================
    $('#mod-tarjeta-10lbs').val('');
    $('#mod-prestamo-10lbs').val('');
    $('#mod-checador-10lbs').val('');
    $('#mod-inasistencias-10lbs').val('');
    $('#mod-permisos-10lbs').val('');
    $('#mod-uniforme-10lbs').val('');
    $('#mod-fagafetcofia-10lbs').val('');
    
    // Historiales
    $('#contenedor-historial-olvidos').empty();
    $('#contenedor-historial-inasistencias-10lbs').empty();
    $('#contenedor-historial-permisos-10lbs').empty();
    $('#contenedor-historial-uniforme-10lbs').empty();
    $('#contenedor-deducciones-adicionales-10lbs').empty();

    // ========================================
    // LIMPIAR: SUELDO A COBRAR
    // ========================================
    $('#mod-redondear-sueldo-10lbs').prop('checked', false);
    $('#mod-sueldo-a-cobrar-10lbs').val('');
}
