mostrarEstructuraPercepciones10lbs();
eliminarPercepcionExtra10lbs();
mostrarEstructuraDeducciones10lbs();
eliminarDeduccionExtra10lbs();
...
function mostrarEstructuraPercepciones10lbs() {
    // Agregar percepciones adicionales
    $("#btn-agregar-percepcion-10lbs").click(function (e) {
        e.preventDefault();
    ...
    `;
        ...
        $('#contenedor-conceptos-adicionales-10lbs').append(nuevoConcepto);

        // Agregar evento change para guardar cambios
        $('#contenedor-conceptos-adicionales-10lbs').find('.percepcion-extra-item').last().find('input').on('change', function () {
            guardarPercepcionesExtra10lbs(empleado);
        });
...
function mostrarPercepcionesExtras10lbs(empleado) {
    ...
    if (empleado.percepciones_extra && Array.isArray(empleado.percepciones_extra) && empleado.percepciones_extra.length > 0) {
        empleado.percepciones_extra.forEach((percepcion) => {
            ...
            $('#contenedor-conceptos-adicionales-10lbs').append(htmlPercepcion);
...
function guardarPercepcionesExtra10lbs(empleado) {
    // Si no hay empleado, salir
    if (!empleado) return;
    ...
    $('#contenedor-conceptos-adicionales-10lbs').find('.percepcion-extra-item').each(function () {
...
function eliminarPercepcionExtra10lbs() {
    // Delegación de eventos para eliminar percepciones
    $("#contenedor-conceptos-adicionales-10lbs").on('click', '.btn-eliminar-percepcion', function () {
        const empleado = objEmpleado.getEmpleado();
...
function mostrarEstructuraDeducciones10lbs() {
    // Agregar deducciones adicionales
    $("#btn-agregar-deduccion-10lbs").click(function (e) {
        e.preventDefault();
    ...
    `;
        ...
        $('#contenedor-deducciones-adicionales-10lbs').append(nuevaDeduccion);

        // Agregar evento change para guardar cambios
        $('#contenedor-deducciones-adicionales-10lbs').find('.deduccion-extra-item').last().find('input').on('change', function () {
            guardarDeduccionesExtra10lbs(empleado);
        });
...
function mostrarDeduccionesExtras10lbs(empleado) {
    ...
    if (empleado.deducciones_extra && Array.isArray(empleado.deducciones_extra) && empleado.deducciones_extra.length > 0) {
        ...
        `;
            $('#contenedor-deducciones-adicionales-10lbs').append(elementoDeduccion);
...
function guardarDeduccionesExtra10lbs(empleado) {
    // Si no hay empleado, salir
    if (!empleado) return;
    ...
    $('#contenedor-deducciones-adicionales-10lbs').find('.deduccion-extra-item').each(function () {
...
function eliminarDeduccionExtra10lbs() {
    // Delegación de eventos para eliminar deducciones
    $("#contenedor-deducciones-adicionales-10lbs").on('click', '.btn-eliminar-deduccion', function () {
        const empleado = objEmpleado.getEmpleado();
...
function guardarInasistenciaManual() {
    $("#btn-agregar-inasistencia-10lbs").click(function (e) {
        e.preventDefault();
        ...
        const dia = $('#select-dia-inasistencia-10lbs').val().trim();
        const descuento = parseFloat($('#input-descuento-inasistencia-10lbs').val()) || 0;
        ...
        // Limpiar formulario
        $('#select-dia-inasistencia-10lbs').val('');
        $('#input-descuento-inasistencia-10lbs').val('0.00');
...
function guardarDescuentoOlvido(inputElement, olvidoObject, empleadoObject) {
    ...
    $('#mod-checador-10lbs').val(total.toFixed(2));
    establecerHistorialChecador(empleadoObject);
    calcularSueldoACobrar();
}
...
function guardarPermisoManual() {
    ...
    $('#input-minutos-permiso-10lbs').on('input', recalcularDescuentoPermiso);
    $('#input-costo-minuto-permiso-10lbs').on('input', recalcularDescuentoPermiso);

    $("#btn-agregar-permiso-10lbs").click(function (e) {
        e.preventDefault();
        ...
        // Obtener valores del formulario
        const dia = $('#select-dia-permiso-10lbs').val().trim();
        const minutos = parseInt($('#input-minutos-permiso-10lbs').val()) || 0;
        const costoMinuto = parseFloat($('#input-costo-minuto-permiso-10lbs').val()) || 0;
        ...
        // Limpiar formulario
        $('#select-dia-permiso-10lbs').val('');
        $('#input-minutos-permiso-10lbs').val('0');
        $('#input-costo-minuto-permiso-10lbs').val('0.00');
        $('#input-descuento-permiso-10lbs').val('0.00');
        ...
        $('#mod-permisos-10lbs').val(totalPermisos.toFixed(2));
...
function recalcularDescuentoPermiso() {
    ...
    const costoMinuto = parseFloat($('#input-costo-minuto-permiso-10lbs').val()) || 0;
    const descuento = minutos * costoMinuto;
    $('#input-descuento-permiso-10lbs').val(descuento.toFixed(2));
}
...
function guardarUniformeManual() {
    $("#btn-agregar-uniforme-10lbs").click(function (e) {
        e.preventDefault();
        const empleado = objEmpleado.getEmpleado();
        if (!empleado) return;

        const folio = $('#input-folio-uniforme-10lbs').val().trim();
        const cantidad = parseInt($('#input-cantidad-uniforme-10lbs').val()) || 0;
        if (!folio) {
        ...
        $('#input-folio-uniforme-10lbs').val('');
        $('#input-cantidad-uniforme-10lbs').val('0');

        // recalcular total
        const total = empleado.historial_uniforme.reduce((s, u) => s + (parseInt(u.cantidad) || 0), 0);
        $('#mod-uniforme-10lbs').val(total);
