...
window.addEventListener('beforeunload', function () {
    if (typeof jsonNominaPalmilla !== 'undefined') {
...
function saveNomina(jsonNominaPalmilla) {
    try {
        ...
        localStorage.setItem('jsonNominaPal', str);
        return true;
    } catch (err) {
        return false;
    }
...
function loadNomina() {
    try {
        const str = localStorage.getItem('jsonNominaPal');
        if (!str) return null;
...
function restoreNomina() {
    try {
        ...
        saveNomina(jsonNominaPalmilla); // Guardar el JSON de la nómina en el local storage
        cambiarVistaTablaNomina(); // Cambiar la vista para mostrar la tabla de nómina
        actualizarCabeceraNomina(jsonNominaPalmilla); // Actualizar la cabecera de la nómina
...
function clearNomina() {
    try {
        localStorage.removeItem('jsonNominaPal');
