...
function establecerDatosTrabajador(clave, nombre) {
    ...
    // Actualizar el nombre en el encabezado del modal
    $('#nombre-empleado-modal').text(nombre);
}
...
function establecerDatosPercepciones(empleado) {
    ...
    $("#mod-horas-extras").off('input').on('input', function () {
        const clave = $('#campo-clave').text().trim();
        const sueldoExtra = parseFloat($(this).val()) || 0;
        actualizarPropiedadEmpleadoEnJsonGlobal(clave, 'sueldo_extra', sueldoExtra);
        actualizarPropiedadEmpleadoEnJsonGlobal(clave, 'sueldo_extra_manual', true); // Activar bandera manual
...
function establecerDatosDeducciones(empleado) {
    ...
    const valorOriginalTarjeta = empleado.neto_pagar_original;
    ...
    $("#mod-tarjeta").off('input').on('input', function () {
        ...
        // Si el valor ingresado es mayor al original y el original no es cero
        if (valorOriginalTarjeta > 0 && valorIngresado > valorOriginalTarjeta) {
            // Establecer automáticamente el valor al máximo permitido
            valorIngresado = valorOriginalTarjeta;
            $(this).val(valorIngresado.toFixed(2));
            
            // Actualizar el último valor válido
            $(this).data('last-valid', valorIngresado);
        } else {
            // Guardar el valor actual como último válido
            $(this).data('last-valid', valorIngresado);
        }
    ...
    if (!empleado.sueldo_base_manual) {
        if (window.rangosHorasJson && window.rangosHorasJson.length > 0) {
            ...
            if (rangoEncontrado) {
                ...
                if (!empleado.sueldo_extra_manual) { // Solo recalcular si no fue modificado manualmente
