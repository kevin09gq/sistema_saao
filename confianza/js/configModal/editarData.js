...
function editarPropiedades() {
    $("#btn-guardar-propiedades-confianza").click(function (e) {
        e.preventDefault();
...
function modificarPercepciones(empleado) {
    ...
    let sueldoSemanal = parseFloat($('#mod-sueldo-semanal-confianza').val());
    let sueldoExtraTotal = parseFloat($('#mod-total-extra-confianza').val());
...
function modificarConceptos(empleado) {
    ...
    actualizarConcepto("45", parseFloat($('#mod-isr-confianza').val()) || 0); // ISR
    actualizarConcepto("52", parseFloat($('#mod-imss-confianza').val()) || 0); // IMSS
    actualizarConcepto("16", parseFloat($('#mod-infonavit-confianza').val()) || 0); // Infonavit
    actualizarConcepto("107", parseFloat($('#mod-ajustes-sub-confianza').val()) || 0); // Ajuste al Sub
...
function actualizarValoresHistorialesEnModal(empleado) {
    ...
    if (!Array.isArray(empleado.historial_inasistencias) || empleado.historial_inasistencias.length === 0) {
        $('#mod-inasistencias-confianza').val(0);
    } else {
    ...
    if (!Array.isArray(empleado.historial_olvidos) || empleado.historial_olvidos.length === 0) {
        $('#mod-checador-confianza').val(0);
    } else {
        $('#mod-checador-confianza').val(empleado.checador || 0);
    }
}
...
function modificarDeducciones(empleado) {
    let tarejta = parseFloat($('#mod-tarjeta-confianza').val());
    let prestamo = parseFloat($('#mod-prestamo-confianza').val());
    let checador = parseFloat($('#mod-checador-confianza').val());
    let retardos = parseFloat($('#mod-retardos-confianza').val());
    let inasistencias = parseFloat($('#mod-inasistencias-confianza').val());
    let uniformes = parseFloat($('#mod-uniforme-confianza').val());
    let permiso = parseFloat($('#mod-permisos-confianza').val());
    let fa_gafet_cofia = parseFloat($('#mod-fagafetcofia-confianza').val());
    ...
    const retardosVacio = $('#mod-retardos-confianza').val() === '';
    const checadorVacio = $('#mod-checador-confianza').val() === '';
    const inasistenciasVacio = $('#mod-inasistencias-confianza').val() === '';
