...
function calcularOlvidosTodosEmpleados(jsonNomina10lbs) {
...
function mostrarEventosPorCampo(empleado, selectorContent, selectorTotal, filtro, textoVacio, tipo = 'entrada') {
    ...
    if (!Array.isArray(empleado.registros) || !Array.isArray(jsonNomina10lbs.horarios_semanales)) {
    ...
    jsonNomina10lbs.horarios_semanales.forEach(horario => {
...
function mostrarEntradasTempranas(empleado) {
    mostrarEventosPorCampo(empleado, '#entradas-tempranas-10lbs', '#total-entradas-tempranas-10lbs',
...
function mostrarSalidasTardias(empleado) {
    mostrarEventosPorCampo(empleado, '#salidas-tardias-10lbs', '#total-salidas-tardias-10lbs',
...
function mostrarSalidasTempranas(empleado) {
    mostrarEventosPorCampo(empleado, '#salidas-tempranas-10lbs', '#total-salidas-tempranas-10lbs',
...
function mostrarRetardos(empleado) {
    mostrarEventosPorCampo(empleado, '#retardos-10lbs', '#total-retardos-10lbs',
...
function mostrarInasistencias(empleado) {
    const $content = $('#inasistencias-content-10lbs');
    const $total = $('#total-inasistencias-10lbs');
    $content.empty();

    if (!Array.isArray(empleado.registros) || !Array.isArray(jsonNomina10lbs.horarios_semanales) || empleado.registros.length === 0) {
    ...
    const horariosPorDia = {};
    jsonNomina10lbs.horarios_semanales.forEach(h => {
...
function mostrarOlvidosChecador(empleado) {
    const $content = $('#olvidos-checador-10lbs');
    $content.empty();
    ...
    $('#total-olvidos-checador-10lbs').text(empleado.historial_olvidos.length);
}
