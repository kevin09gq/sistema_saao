...
function guardarNomina($data, $conexion)
{
    ...
    $query = "SELECT * FROM nomina_10lbs WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
    ...
    if ($result->num_rows > 0) {
        if ($actualizar) {
            ...
            $updateQuery = "UPDATE nomina_10lbs SET nomina_10lbs = ?, total_percepciones = ?, total_deducciones = ?, total_neto = ? WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
        ...
        $insertQuery = "INSERT INTO nomina_10lbs (id_empresa, numero_semana, anio, nomina_10lbs, total_percepciones, total_deducciones, total_neto) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
function validarExistenciaNomina($data, $conexion)
{
    ...
    $query = "SELECT COUNT(*) AS cnt FROM nomina_10lbs WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
    $stmt = $conexion->prepare($query);
    $stmt->bind_param("iii", $id_empresa, $numero_semana, $anio);
    $stmt->execute();
    $result = $stmt->get_result();
...
// Obtener la nómina almacenada
function obtenerNomina($data, $conexion)
{
    $numero_semana = isset($data['numero_semana']) ? intval($data['numero_semana']) : 0;
    $id_empresa = isset($data['id_empresa']) ? intval($data['id_empresa']) : 1;

    // Usar la tabla `nomina_10lbs`
    $anio = isset($data['anio']) ? intval($data['anio']) : 0;
    $query = "SELECT nomina_10lbs FROM nomina_10lbs WHERE id_empresa = ? AND numero_semana = ? AND anio = ? ORDER BY id_nomina_10lbs DESC LIMIT 1";
    $stmt = $conexion->prepare($query);
    if (!$stmt) {
