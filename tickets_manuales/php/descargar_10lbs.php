...
function renderTicketPdf(TCPDF $pdf, $emp, $extra, $meta) {
    ...
    if (isset($emp['percepciones_manuales'])) {
        foreach ($emp['percepciones_manuales'] as $p) {
            if (toNumber($p['monto'] ?? 0) > 0) {
                $percepciones[] = ['label' => $conceptoNum++ . ' ' . safeText($p['nombre'] ?? ''), 'monto' => toNumber($p['monto'])];
            }
        }
    }
    ...
    if (isset($emp['deducciones_manuales'])) {
        foreach ($emp['deducciones_manuales'] as $d) {
            if (toNumber($d['monto'] ?? 0) > 0) {
                $deducciones[] = ['label' => safeText($d['nombre'] ?? ''), 'monto' => toNumber($d['monto'])];
            }
        }
    }

    $totalPerp = 0.0;
    foreach ($percepciones as $p) $totalPerp += toNumber($p['monto']);
    $totalDed = 0.0;
    foreach ($deducciones as $d) $totalDed += toNumber($d['monto']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['datos'])) {
    ...
    $pdf->Output('Ticket_10lbs_' . $emp['clave'] . '.pdf', 'D');
}
