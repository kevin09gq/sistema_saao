...
function renderTicketPdf(TCPDF $pdf, $emp, $extra, $meta) {
    ...
    $netoOriginal = $totalPerp - $totalDed;
    ...
    if ($ajusteRedondeo > 0) {
        ...
        $totalPerp += $ajusteRedondeo;
    } elseif ($ajusteRedondeo < 0) {
        $deducciones[] = ['label' => 'Redondeo', 'monto' => abs($ajusteRedondeo)];
        $totalDed += abs($ajusteRedondeo);
    }
    ...
    for ($i = 0; $i < $maxConceptos; $i++) {
        ...
        if ($i == 0) {
            ...
            $text(310, 22, $pt(14), $departamento);
            ...
            $textB(118, 75, $pt(17), $emp['dias_trabajados'] ?? '0');
    ...
    $text(310, $yTotales + 13, $pt(16), '$' . money($totalPerp));
    $text(430, $yTotales + 13, $pt(16), 'Total Deducciones');
    $text(710, $yTotales + 13, $pt(16), '-$' . money($totalDed));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['datos'])) {
    ...
    $pdf->Output('Ticket_Palmilla_' . $emp['clave'] . '.pdf', 'D');
}
