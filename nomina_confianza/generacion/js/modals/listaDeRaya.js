...
$(document).ready(function () {
    ...
    // Procesar archivo de Cítricos SAAO
    obtenerInformacionListaDeRaya();

    // Procesar archivo de SB Group
    obtenerInformacionListaDeRayaSBGroup();
});
...
// FUNCIÓN PARA COPIAR LA INFORMACIÓN DEL EMPLEADO
// BUSCA EL EMPLEADO DENTRO DE jsonNominaConfianza
...
//===================================================
// FUNCIÓN PARA LEER EL ARCHIVO DE EXCEL — SB GROUP
// Funciona exactamente igual que obtenerInformacionListaDeRaya()
// pero usa el input y botón de SB Group.
// Llama al mismo leerListaRaya.php y a la misma función
// de comparación, por lo que la lógica es idéntica.
//===================================================

function obtenerInformacionListaDeRayaSBGroup() {

    $('#btnProcesarListaRayaSBGroup').on('click', function (e) {
        e.preventDefault();

        // Obtener el archivo que seleccionó el usuario (sección SB Group)
        let archivo = $('#inputArchivoListaRayaSBGroup')[0].files[0];

        // Verificar que el usuario haya seleccionado un archivo
        if (!archivo) {
            alert('Seleccione un archivo de Excel para SB Group.');
            return;
        }
        ...
        // Agregar el archivo con el mismo nombre que espera el PHP:
        // $_FILES['archivo_excel']
        datos.append('archivo_excel', archivo);

        Swal.fire({

            title: 'Procesando archivo SB Group...',
        ...
        // Enviar el archivo al mismo PHP que procesa la lista de raya
        $.ajax({

            // Mismo archivo PHP — la lógica de lectura del Excel es igual
            url: '../php/leerListaRaya.php',
            ...
            success: function (respuesta) {
                ...
                // Comparar los empleados seleccionados con los empleados
                // obtenidos del archivo de SB Group.
                // Se reutiliza la misma función de comparación.
                compararEmpleadosListaDeRaya(empleadosSeleccionadosListaRaya, respuesta.empleados);
