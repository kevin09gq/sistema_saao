...
class PDFEncabezado extends TCPDF
{
    ...
    public function Header()
    {
        ...
        $this->SetY(12);
        $this->Cell(0, 10, 'REPORTE NÓMINA CONFIANZA', 0, 1, 'C', 0, '', 0, false, 'M', 'M');
...
function mesEnLetras($mes)
{
    ...
    return isset($meses[$mes]) ? $meses[$mes] : mb_strtoupper($mes, 'UTF-8');
}
...
if (isset($datosNomina['departamentos']) && is_array($datosNomina['departamentos'])) {
    foreach ($datosNomina['departamentos'] as $depto) {
        $nombreDepto = isset($depto['nombre']) ? mb_strtoupper(trim($depto['nombre']), 'UTF-8') : 'SIN DEPARTAMENTO';
        $empleadosValidos = [];
...
$pdf->SetCreator('SAAO');
$pdf->SetAuthor('SAAO');
$pdf->SetTitle('Reporte Contable de Nómina Confianza');
...
foreach ($empleadosPorTipo as $empleado) {
    ...
    $nombre = mb_strtoupper($empleado['nombre'] ?? '', 'UTF-8');
    ...
    // --- TABLAS DE DETALLES DE HISTORIALES (DESPUÉS DE PERCEPCIONES Y DEDUCCIONES) ---

    // 1. Ausentismos (Inasistencias)
    if (!empty($empleado['historial_inasistencias']) && is_array($empleado['historial_inasistencias'])) {
        $pdf->Ln(6);
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->SetFillColor(230, 230, 230);
        $pdf->Cell(190, 7, 'HISTORIAL DE INASISTENCIAS / AUSENTISMOS', 0, 1, 'C', true);
        ...
        $totDescuentoInas = 0;
        foreach ($empleado['historial_inasistencias'] as $hInas) {
            $dia = !empty($hInas['dia']) ? $hInas['dia'] : (!empty($hInas['fecha']) ? $hInas['fecha'] : '-');
            $minutos = isset($hInas['minutos']) ? $hInas['minutos'] . ' min' : '-';
            $costoMin = formatoMoneda($hInas['costo_por_minuto'] ?? 0);
            $monto = (float) ($hInas['descuento_inasistencia'] ?? 0);
            $totDescuentoInas += $monto;
        ...
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetFillColor(240, 240, 240);
        $pdf->Cell(140, 6, 'TOTAL INASISTENCIAS', 1, 0, 'R', true);
        $pdf->SetFont('dejavusansmono', 'B', 9);
        $pdf->Cell(50, 6, formatoMoneda($totDescuentoInas), 1, 1, 'R', true);
    }

    // 2. Permisos
    if (!empty($empleado['historial_permisos']) && is_array($empleado['historial_permisos'])) {
        $pdf->Ln(4);
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->SetFillColor(230, 230, 230);
        ...
        $pdf->SetFont('dejavusans', '', 9);
        $totDescuentoPerm = 0;
        foreach ($empleado['historial_permisos'] as $hPerm) {
    ...
    // 3. Retardos
    if (!empty($empleado['historial_retardos']) && is_array($empleado['historial_retardos'])) {
        $pdf->Ln(4);
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->SetFillColor(230, 230, 230);
        $pdf->Cell(190, 7, 'HISTORIAL DE RETARDOS', 0, 1, 'C', true);
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetFillColor(245, 245, 245);
        $pdf->Cell(40, 6, 'DÍA', 1, 0, 'C', true);
        $pdf->Cell(35, 6, 'MINUTOS RETARDO', 1, 0, 'C', true);
        $pdf->Cell(30, 6, 'TOLERANCIA', 1, 0, 'C', true);
        $pdf->Cell(40, 6, 'COSTO X MINUTO', 1, 0, 'C', true);
        $pdf->Cell(45, 6, 'DESCUENTO', 1, 1, 'C', true);

        $pdf->SetFont('dejavusans', '', 9);
        $totDescuentoRetardos = 0;
        foreach ($empleado['historial_retardos'] as $hRet) {
            $dia = !empty($hRet['dia']) ? $hRet['dia'] : (!empty($hRet['fecha']) ? $hRet['fecha'] : '-');
            $minutos = isset($hRet['minutos_retardo']) ? $hRet['minutos_retardo'] . ' min' : '-';
            $tolerancia = isset($hRet['tolerancia']) ? $hRet['tolerancia'] . ' min' : '-';
            $costoMin = formatoMoneda($hRet['descuento_por_minuto'] ?? $hRet['costo_por_minuto'] ?? 0);
            $monto = (float) ($hRet['total_descontado'] ?? $hRet['descuento_retardo'] ?? 0);
            $totDescuentoRetardos += $monto;
            $descuento = formatoMoneda($monto);

            $pdf->Cell(40, 6, $dia, 1, 0, 'C');
            $pdf->Cell(35, 6, $minutos, 1, 0, 'C');
            $pdf->Cell(30, 6, $tolerancia, 1, 0, 'C');
            $pdf->Cell(40, 6, $costoMin, 1, 0, 'C');
            $pdf->SetFont('dejavusansmono', '', 9);
            $pdf->Cell(45, 6, $descuento, 1, 1, 'R');
            $pdf->SetFont('dejavusans', '', 9);
        }
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetFillColor(240, 240, 240);
        $pdf->Cell(145, 6, 'TOTAL RETARDOS', 1, 0, 'R', true);
        $pdf->SetFont('dejavusansmono', 'B', 9);
        $pdf->Cell(45, 6, formatoMoneda($totDescuentoRetardos), 1, 1, 'R', true);
    }

    // 4. Uniformes
    if (!empty($empleado['historial_uniforme']) && is_array($empleado['historial_uniforme'])) {
        $pdf->Ln(4);
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->SetFillColor(230, 230, 230);
        ...
        $pdf->SetFont('dejavusans', '', 9);
        $totDescuentoUnif = 0;
        foreach ($empleado['historial_uniforme'] as $hUnif) {
    ...
    // 5. Olvidos de Biométrico
    if (!empty($empleado['historial_olvidos']) && is_array($empleado['historial_olvidos'])) {
        $pdf->Ln(4);
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->SetFillColor(230, 230, 230);
        ...
        $pdf->SetFont('dejavusans', '', 9);
        $totDescuentoOlv = 0;
        foreach ($empleado['historial_olvidos'] as $hOlv) {
