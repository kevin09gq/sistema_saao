...
function establerDataModal(empleado) {
    ...
    const modalEl = document.getElementById('modal-confianza');
...
function establecerInformacionEmpleado(empleado) {
    ...
    $('#campo-clave-confianza').text(empleado.clave || '');
    $('#campo-nombre-confianza').text(empleado.nombre || '');
...
function establecerBiometrico(empleado) {
    ...
    empleado.registros.forEach(registro => {
        ...
        $('#tbody-biometrico-confianza').append(fila);
    });
}
...
function establecerColorBiometrico(empleado) {
    ...
    $('#tbody-biometrico-confianza tr').each(function () {
...
function establecerHorariosOficiales(empleado) {
    ...
    horarios.forEach(horario => {
                ...
                <td>${horario.salida || '-'}</td>
                 <td>
                    <div class="d-flex gap-1 align-items-center">
                        <button type="button" class="btn btn-outline-primary btn-sm btn-justificado-confianza" title="Abrir modal">
                            <i class="bi bi-clipboard2-check"></i>
                        </button>
                        <button type="button" class="btn btn-outline-danger btn-sm btn-eliminar-horario-confianza" title="Eliminar">
        ...
        // Agregar fila a la tabla
        $('#tbody-horarios-oficiales-confianza').append(fila);
    });
}
...
function establecerPercepciones(empleado) {
    ...
    $('#mod-sueldo-semanal-confianza').val(empleado.salario_semanal || '');
    // Establecer sueldo extra total
    $('#mod-total-extra-confianza').val(empleado.sueldo_extra_total || '');
...
function establecerConceptos(empleado) {
    ...
    $('#mod-isr-confianza').val(conceptoISR ? conceptoISR.resultado || '' : '');
    $('#mod-imss-confianza').val(conceptoIMSS ? conceptoIMSS.resultado || '' : '');
    $('#mod-infonavit-confianza').val(conceptoInfonavit ? conceptoInfonavit.resultado || '' : '');
    $('#mod-ajustes-sub-confianza').val(conceptoAjusteSub ? conceptoAjusteSub.resultado || '' : '');
...
function calcularTotalConceptos() {
    ...
    const ajusteSub = parseFloat($('#mod-ajustes-sub-confianza').val()) || 0;
...
function desabilitarCamposConceptos(tieneSeguroSocial) {
    ...
    if (!tieneSeguroSocial) {
        ...
        $('#mod-isr-confianza').prop('disabled', true);
    ...
    $('#mod-isr-confianza').prop('disabled', false);
...
function establecerDeducciones(empleado) {
    ...
    // Establecer tarjeta 
    $('#mod-tarjeta-confianza').val(empleado.tarjeta || '');
    // Establecer préstamo
    $('#mod-prestamo-confianza').val(empleado.prestamo || '');
    // Establecer checador
    $('#mod-checador-confianza').val(empleado.checador || '');
    // Establecer retardos
    $('#mod-retardos-confianza').val(empleado.retardos || '');
    // Establecer inasistencias
    $('#mod-inasistencias-confianza').val(empleado.inasistencia || '');
    // Establecer permisos 
    $('#mod-permisos-confianza').val(empleado.permiso || '');
    // Establecer uniforme 
    $('#mod-uniforme-confianza').val(empleado.uniformes || '');

    // Establecer deducciones extras (depende del array)
    mostrarDeduccionesExtras(empleado);
    $('#mod-fagafetcofia-confianza').val(empleado.fa_gafet_cofia || '');

    // Restaurar estado del redondeo (si estaba activo al guardar)
    const redondeoActivo = empleado.redondeo_activo === true;
    $('#mod-redondear-sueldo-confianza').prop('checked', redondeoActivo);
    if (redondeoActivo) {
