...
$clavesExcluidas = $data["claves_excluidas"] ?? []; // Claves compuestas "clave|id_empresa" de empleados que ya están en la lista de raya
...
if (!empty($biometricos)) {
    ...
    // pero excluyendo los que ya están en la lista de raya por clave compuesta
    
    if (!empty($clavesExcluidas)) {
        // Parsear claves compuestas "clave|id_empresa"
        $exclusiones = [];
        foreach ($clavesExcluidas as $claveCompuesta) {
            $partes = explode('|', $claveCompuesta);
            if (count($partes) === 2) {
                $exclusiones[] = [
                    'clave' => trim($partes[0]),
                    'id_empresa' => intval($partes[1])
                ];
            }
        }
        
        // Obtener todos los empleados sin IMSS
        $sql = $conexion->prepare(
            "SELECT clave_empleado, nombre, ap_paterno, ap_materno, salario_semanal, salario_diario, id_departamento, id_empresa, biometrico FROM info_empleados 
             WHERE id_status = 1 
               AND (imss = '' OR status_nss = 0)
        ...
        // Filtrar en PHP por clave compuesta
        while ($row = $res->fetch_assoc()) {
            $esExcluido = false;
            foreach ($exclusiones as $excl) {
                if ($row['clave_empleado'] === $excl['clave'] && intval($row['id_empresa']) === $excl['id_empresa']) {
                    $esExcluido = true;
                    break;
                }
            }
