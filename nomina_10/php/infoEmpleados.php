...
switch ($accion) {
    ...
    case 'obtenerPreciosCajas':
        obtenerPreciosCajas($conexion);
        break;
...
function obtenerInfoDepartamento(mysqli $conexion)
{
    ...
    $sql = "SELECT
                ...
                CASE 
                    WHEN (
                        SELECT COUNT(DISTINCT nd2.id_empresa) 
                        FROM nomina_departamento nd2 
                        WHERE nd2.id_departamento = d.id_departamento
                    ) > 1 THEN CONCAT(d.nombre_departamento, ' - ', e.nombre_empresa)
                    ELSE d.nombre_departamento
            ...
            INNER JOIN empresa e
                ON nd.id_empresa = e.id_empresa
            WHERE nd.id_nomina = 2";
...
// FUNCIÓN PARA OBTENER LOS PRECIOS DE LAS CAJAS
// UTILIZADAS EN LA NÓMINA 10 LBS. ESTOS PRECIOS SE 
// UTILIZAN PARA CALCULAR LA UTILIDAD DE CADA EMPLEADO
function obtenerPreciosCajas(mysqli $conexion)
{
    $sql = "SELECT
                valor,
                precio,
    ...
    if (!$resultado) {
        echo json_encode([
            "success" => false,
            "mensaje" => "Error al obtener los precios de las cajas: " . $conexion->error
        ]);
        return;
    }
...
function guardarNomina(mysqli $conexion)
{
    ...
    $sqlCheck = "SELECT id_nomina_10lbs FROM nomina_10lbs 
    ...
    if ($resultadoCheck->num_rows > 0) {
        ...
        $idNomina = $fila['id_nomina_10lbs'];

        $sqlUpdate = "UPDATE nomina_10lbs 
                      SET nomina_10lbs = ?, 
                          total_percepciones = ?, 
                          total_deducciones = ?, 
                          total_neto = ?
        ...
        $sqlInsert = "INSERT INTO nomina_10lbs 
                      (id_empresa, anio, numero_semana, nomina_10lbs, total_percepciones, total_deducciones, total_neto)
                      VALUES (?, ?, ?, ?, ?, ?, ?)";
...
function recuperarUltimaNomina(mysqli $conexion)
{
    ...
    $sql = "SELECT id_nomina_10lbs, anio, numero_semana, nomina_10lbs,
                   total_percepciones, total_deducciones, total_neto
            FROM nomina_10lbs
            WHERE id_empresa = ?
            ORDER BY id_nomina_10lbs DESC
            LIMIT 1";
    ...
    echo json_encode([
        "success" => true,
        "nomina_json" => $fila['nomina_10lbs'],
        "nomina_info" => [
            "id_nomina_10lbs" => $fila['id_nomina_10lbs'],
            "anio" => $fila['anio'],
