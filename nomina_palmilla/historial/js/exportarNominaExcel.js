...
function cargarDepartamentosExportar() {
    ...
    if (!jsonHistorialPalmilla || !Array.isArray(jsonHistorialPalmilla.departamentos)) {
        container.html('<div class="alert alert-warning small">No hay departamentos disponibles</div>');
        return;
    }

    jsonHistorialPalmilla.departamentos.forEach(depto => {
...
function exportarNominaDepartamento() {
    $(document).on('click', '#btn-export-corte', function (e) {
        ...
        // Validar que jsonHistorialPalmilla exista
        if (!jsonHistorialPalmilla) {
            alert('No hay datos de nómina para exportar. Por favor, procesa los datos primero.');
            return;
        }

        // Validar que el departamento exista
        const departamento = jsonHistorialPalmilla.departamentos.find(d => d.id_departamento == deptoId);
        ...
        // Enviar el jsonHistorialPalmilla al servidor PHP mediante POST
        $.ajax({
            url: tmp_url,
            type: 'POST',
            data: {
                jsonNomina: JSON.stringify(jsonHistorialPalmilla),
                deptoId: deptoId,
            ...
            success: function (blob) {
                ...
                link.href = url;
                var numeroSemana = String(jsonHistorialPalmilla.numero_semana).padStart(2, '0');
                var aniosCierre = jsonHistorialPalmilla.fecha_cierre.split('/')[2];
    ...
    $(document).on('click', '#btn-export-nomina-completa', function (e) {
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                var aniosCierre = jsonHistorialPalmilla.fecha_cierre.split('/')[2];
                var timestamp = new Date().toISOString().replace(/T/, '_').replace(/:/g, '-').split('.')[0];
                link.download = 'SEM ' + numeroSemana + ' - ' + 'RANCHO PALMILLA - ' + aniosCierre + '.xlsx';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
...
function reporteNominaPdf() {
    $("#btn_export_pdf_reporte").click(function (e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                numero_semana: jsonHistorialPalmilla.numero_semana || '',
                fecha_cierre: jsonHistorialPalmilla.fecha_cierre || '',
                fecha_inicio: jsonHistorialPalmilla.fecha_inicio || '',
                periodo_nomina: jsonHistorialPalmilla.periodo_nomina || '',
                jsonNomina: JSON.stringify(jsonHistorialPalmilla)
            },
            xhrFields: {
                responseType: 'blob'
    ...
    $("#btn-export-dispersion-tarjeta").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                link.href = url;
                var numeroSemana = String(jsonHistorialPalmilla.numero_semana).padStart(2, '0');
                var anio = jsonHistorialPalmilla.fecha_cierre.split('/')[2];
                link.download = 'SEM_' + numeroSemana + '_DISPERSION_TARJETA_PALMILLA_' + anio + '.xlsx';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
...
function validarEmpleadosNegativos() {
    ...
    if (!jsonHistorialPalmilla || !jsonHistorialPalmilla.departamentos) {
        return false;
    }
