...
try {
    if ($action === 'get_filtros_iniciales') {
        $query = "
            ...
            JOIN podas_palmilla p ON n.id_nomina_palmilla = p.id_nomina
    ...
    if ($action === 'buscar') {
        ...
        $color_area = '#004D17'; // Color por defecto (verde institucional)
        $query_color = "
            SELECT a.colores 
            FROM areas a
        ...
        $query_podas = "
            SELECT 
                ...
                n.numero_semana as sem_poda, n.anio as anio_poda, n.nomina_palmilla,
        ...
        while ($p = $result_podas->fetch_assoc()) {
            $mes_poda = extraerMesDesdeJson($p['nomina_palmilla']);
            if ($filtro_mes !== '' && (int)$mes_poda !== (int)$filtro_mes) {
                continue;
            }

            unset($p['nomina_palmilla']);
