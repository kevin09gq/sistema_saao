...
switch ($case) {
    case 'guardarNominaPalmilla':
        guardarNominaPalmilla($data, $conexion);
        break;
...
function guardarNominaPalmilla($data, $conexion)
{
    ...
    $query = "SELECT * FROM nomina_palmilla WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
    ...
    if ($result->num_rows > 0) {
        if ($actualizar) {
            ...
            $updateQuery = "UPDATE nomina_palmilla SET nomina_palmilla = ?, total_percepciones = ?, total_deducciones = ?, total_neto = ? WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
            ...
            if ($updateStmt->execute()) {
                ...
                $queryId = "SELECT id_nomina_palmilla FROM nomina_palmilla WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
                $stmtId = $conexion->prepare($queryId);
                $stmtId->bind_param("iii", $id_empresa, $numero_semana, $anio);
                $stmtId->execute();
                $resultId = $stmtId->get_result();
                $rowId = $resultId->fetch_assoc();
                $idNominaActualizada = $rowId['id_nomina_palmilla'];
        ...
        $insertQuery = "INSERT INTO nomina_palmilla (id_empresa, numero_semana, anio, nomina_palmilla, total_percepciones, total_deducciones, total_neto) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
function validarExistenciaNomina($data, $conexion)
{
    ...
    $query = "SELECT COUNT(*) AS cnt FROM nomina_palmilla WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
    $stmt = $conexion->prepare($query);
    $stmt->bind_param("iii", $id_empresa, $numero_semana, $anio);
    $stmt->execute();
    $result = $stmt->get_result();
...
// Obtener la nómina almacenada (tabla `nomina_palmilla` según tu esquema)
function obtenerNomina($data, $conexion)
{
    $numero_semana = isset($data['numero_semana']) ? intval($data['numero_semana']) : 0;
    $id_empresa = isset($data['id_empresa']) ? intval($data['id_empresa']) : 1;

    // Usar la tabla `nomina_palmilla` que contiene la columna `anio` según tu esquema
    $anio = isset($data['anio']) ? intval($data['anio']) : 0;
    $query = "SELECT id_nomina_palmilla, nomina_palmilla FROM nomina_palmilla WHERE id_empresa = ? AND numero_semana = ? AND anio = ? ORDER BY id_nomina_palmilla DESC LIMIT 1";
    $stmt = $conexion->prepare($query);
    if (!$stmt) {
    ...
    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $nomina_raw = $row['nomina_palmilla'];
        $idNomina = $row['id_nomina_palmilla'];
...
function guardarTicketsCorte($corte, $idNomina, $conexion)
{
    try {
        ...
        $deleteTablesQuery = "DELETE FROM cortes_palmilla_tablas WHERE id_corte IN (SELECT id FROM cortes_palmilla WHERE id_nomina = ?)";
        ...
        $deleteQuery = "DELETE FROM cortes_palmilla WHERE id_nomina = ?";
        ...
        foreach ($empleadosCorte as $empleado) {
            ...
            foreach ($empleado['tickets'] as $ticket) {
                ...
                // Insertar el ticket principal en cortes_palmilla
                $insertCorteQuery = "INSERT INTO cortes_palmilla (id_nomina, nombre_cortador, folio, precio_reja, fecha_corte) VALUES (?, ?, ?, ?, ?)";
                ...
                if ($insertCorteStmt->execute()) {
                    ...
                    foreach ($ticket['datosRejas'] as $datosTabla) {
                        ...
                        $insertTablaQuery = "INSERT INTO cortes_palmilla_tablas (id_corte, num_tabla, rejas) VALUES (?, ?, ?)";
        ...
        foreach ($empleadosCorte as $empleado) {
            ...
            if ($empleado['concepto'] === 'REJA' && isset($empleado['tickets']) && is_array($empleado['tickets'])) {
                foreach ($empleado['tickets'] as $ticket) {
                    ...
                    $checkQuery = "SELECT id, id_nomina FROM cortes_palmilla WHERE folio = ? AND nombre_cortador = ?";
                    ...
                    if ($row = $result->fetch_assoc()) {
                        ...
                        if (is_null($idNominaExistente)) {
                            ...
                            $updateQuery = "UPDATE cortes_palmilla 
                            ...
                            $conexion->query("DELETE FROM cortes_palmilla_tablas WHERE id_corte = $idCorte");
                            foreach ($ticket['datosRejas'] as $datosTabla) {
                                $numTabla = intval($datosTabla['tabla']);
                                $cantidadRejas = intval($datosTabla['cantidad']);
                                $insertTablaQuery = "INSERT INTO cortes_palmilla_tablas (id_corte, num_tabla, rejas) VALUES (?, ?, ?)";
...
function obtenerTicketsCorte($data, $conexion)
{
    ...
    $queryNomina = "SELECT id_nomina_palmilla FROM nomina_palmilla WHERE id_empresa = ? AND numero_semana = ? AND anio = ? ORDER BY id_nomina_palmilla DESC LIMIT 1";
    ...
    if ($resultNomina && $resultNomina->num_rows > 0) {
        ...
        $idNomina = $rowNomina['id_nomina_palmilla'];
...
function obtenerTicketsCorteInterno($idNomina, $conexion)
{
    ...
    $queryCortes = "SELECT id, nombre_cortador, folio, precio_reja, fecha_corte FROM cortes_palmilla WHERE id_nomina = ? ORDER BY nombre_cortador, fecha_corte";
    ...
    while ($corte = $resultCortes->fetch_assoc()) {
        ...
        $queryTablas = "SELECT num_tabla, rejas FROM cortes_palmilla_tablas WHERE id_corte = ? ORDER BY num_tabla";
