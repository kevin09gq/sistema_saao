...
function actualizarPasajeTardeadaEnEmpleados() {
    ...
    // Recolectar empleados del departamento 21 que tengan registros
    ...
    if (Array.isArray(jsonNominaPalmilla.departamentos)) {
        jsonNominaPalmilla.departamentos.forEach(departamento => {
            ...
            departamento.empleados.forEach(empleado => {
                // Solo procesar departamento 21 con registros
                if (parseInt(empleado.id_departamento) === 21 && Array.isArray(empleado.registros)) {
...
function listarEmpleadosParaQuitarComidaPasaje() {
    ...
    // Iterar y filtrar jornaleros (departamento 21 con mostrar = true)
    jsonNominaPalmilla.departamentos.forEach(departamento => {
        if (!Array.isArray(departamento.empleados)) return;

        departamento.empleados.forEach(empleado => {
            if (parseInt(empleado.id_departamento) === 21 && empleado.mostrar !== false) {
                const fila = `
                    <tr>
