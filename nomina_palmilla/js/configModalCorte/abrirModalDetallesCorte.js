...
$(document).on('click', '#context_menu_corte', function (e) {
    ...
    switch (concepto) {
        case 'REJA':
            inicializar_modal_corte_rejas_detalles(jsonNominaPalmilla, nombre, precio);
            break;
        case 'NOMINA':
            inicializar_modal_corte_nomina_detalles(jsonNominaPalmilla, nombre);
            break;
        default:
...
$(btn_guardar_cambios_nomina_corte).on('click', function (e) {
    ...
    const actualizado = actualizarPagosEmpleado(jsonNominaPalmilla, nombreEmpleado, nuevosPageos);
...
$(btn_guardar_cambios_reja_corte).on('click', function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            // Actualizar el empleado en el JSON
            const actualizado = actualizarTicketsEmpleado(jsonNominaPalmilla, nombreEmpleado, precioOriginal, nuevosTickets);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                const departamentoCorte = jsonNominaPalmilla.departamentos.find(dept => dept.nombre === 'Corte');
