/**
 * ================================================================
 * MÓDULO DE SELECCIÓN DE EMPLEADOS PARA TICKETS - NÓMINA PALMILLA
 * ================================================================
...
let filtroDepartamentoActivoPalmilla = 'todos';
...
$(document).ready(function() {
    $('#btn_ticket_seleccion').on('click', function(e) {
        ...
        filtroDepartamentoActivoPalmilla = 'todos';
        $('#filtro_departamento_ticket_palmilla').val('todos');
    ...
    $('#filtro_departamento_ticket_palmilla').on('change', function() {
        filtroDepartamentoActivoPalmilla = $(this).val();
        $('#buscar_empleado_ticket').trigger('input');
    });
    ...
    $('#btn_marcar_visibles_tickets').on('click', function() {
        ...
        empleadosParaTickets.forEach(emp => {
            ...
            if (filtroDepartamentoActivoPalmilla !== 'todos') {
                coincideDepto = (depto === filtroDepartamentoActivoPalmilla.toLowerCase());
            }
    ...
    $(document).on('input', '#buscar_empleado_ticket', filtrarEmpleadosPalmilla);
...
function cargarEmpleadosParaTickets(nominaData) {
    ...
    nominaData.departamentos.forEach(depto => {
        ...
        depto.empleados.forEach(emp => {
            ...
            if (nombreDepto.toUpperCase() === 'CORTE') {
                ...
                ref.movimientosPoda.push(...emp.movimientos.filter(m => m.concepto === 'PODA'));
                ref.extrasPoda.push(...emp.movimientos.filter(m => m.concepto !== 'PODA'));
            } else {
    ...
    const selectDepto = $('#filtro_departamento_ticket_palmilla');
    if (selectDepto.length) {
        ...
        if (departamentosUnicos.includes(filtroDepartamentoActivoPalmilla)) {
            selectDepto.val(filtroDepartamentoActivoPalmilla);
        } else {
            filtroDepartamentoActivoPalmilla = 'todos';
...
function filtrarEmpleadosPalmilla() {
    const query = String($('#buscar_empleado_ticket').val() || '').toLowerCase().trim();
...
function generarTicketsSeleccionados() {
    ...
    let numSemana = '';
    try {
        if (typeof jsonNominaPalmilla !== 'undefined') numSemana = jsonNominaPalmilla.numero_semana || '';
    } catch (e) {}
...
function generarTicketsNombreSeleccionados() {
    ...
    let nombreArchivo;
    if (multiplesDepartamentos) {
        nombreArchivo = `NOMBRE_SEM_${numSemana}_RANCHO PALMILLA_${año}.pdf`;
        departamento = ''; // No enviar departamento si hay múltiples
    } else {
        nombreArchivo = `NOMBRE_SEM_${numSemana}_${departamento}_RANCHO PALMILLA_${año}.pdf`;
    }
