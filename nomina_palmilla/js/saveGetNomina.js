...
function confirmarsaveNomina() {
    $('#btn_guardar_nomina_palmilla').on('click', function () {
        Swal.fire({
            title: '¿Confirmar guardado?',
            text: `¿Está seguro que desea guardar la nómina de la semana ${jsonNominaPalmilla.numero_semana}?`,
            icon: 'question',
...
function saveNominaPalmilla() {
   ...
   // Obtener el dep Corte por separado
    ...
    const departamentoPoda = jsonNominaPalmilla.departamentos.find(d => d.nombre === "Poda");
...
function getNominaPalmilla(numeroSemana, anio) {
    ...
    return $.ajax({
        ...
        try {
            ...
            console.error('getNominaPalmilla parse error', e, resp);
            return null;
        }
    }).catch(function (err) {
