...
function crearEstructuraJson() {
   $("#container-acceso-palmilla").removeAttr("hidden");
    $('#btn_crear_nomina_palmilla').on('click', function () {
        ...
        var semana = $('#semana_nomina_palmilla').val();
        var fechaInicio = $('#fecha_inicio_nomina_palmilla').val();
        var fechaCierre = $('#fecha_cierre_nomina_palmilla').val();
        ...
        jsonNominaPalmilla = {
            ...
            departamentos: [
                {
                    nombre: "palmilla coordinadores",
                    empleados: []
                },
                {
                    nombre: "palmilla jornaleros",
                    empleados: []
                }
            ]
        };

        obtenerJornalerosCoordinadores(jsonNominaPalmilla);
    });
}
...
function obtenerJornalerosCoordinadores(jsonNominaPalmilla) {
    $.ajax({
        ...
        success: function (response) {
            if (response.empleados && response.empleados.length > 0) {
                ...
                response.empleados.forEach(function (empleadoBD) {
                    ...
                    // Agregar horario_oficial y salario_semanal solo para departamento 14 (palmilla coordinadores)
                    if (empleadoBD.id_departamento == 14) {
                    ...
                    let nombreDpto = null;
                    if (empleadoBD.id_departamento == 14) {
                        nombreDpto = 'palmilla coordinadores';
                    } else if (empleadoBD.id_departamento == 21) {
                        nombreDpto = 'palmilla jornaleros';
                    }

                    // Si se determina un departamento válido, agregar el empleado
                    if (nombreDpto && jsonNominaPalmilla && jsonNominaPalmilla.departamentos) {
                        let dpto = jsonNominaPalmilla.departamentos.find(d => d.nombre === nombreDpto);
                        ...
                        if (!dpto) {
                            ...
                            jsonNominaPalmilla.departamentos.push(dpto);
                        }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.existentes) {
                ...
                verificarEmpleadosSinSeguro(jsonNominaPalmilla); // Verificar y agregar empleados sin seguro que no están en la nómina recuperada
        ...
        success: function (response) {
            if (response.empleados && response.empleados.length > 0) {
                ...
                empleadosSinSeguro.forEach(function (empSinSeguro) {
                    ...
                    // Agregar horario_oficial y salario_semanal solo para departamento 14 (coordinadores)
                    if (empleado.id_departamento == 14) {
                    ...
                    if (empleado.id_departamento == 14) {
                        nombreDpto = 'palmilla coordinadores';
                    } else if (empleado.id_departamento == 21) {
                        nombreDpto = 'palmilla jornaleros';
                    }
                ...
                // Filtrar por departamento 14 (coordinadores) por defecto
    ...
    jsonNominaPalmilla.departamentos.forEach(departamento => {
        ...
        departamento.empleados.forEach(empleado => {
            ...
            */
            ...
            if (["21"].includes(empleado.id_departamento)) {
