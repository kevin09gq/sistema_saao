    ...
    fetch('../php/historial/get_historial_podas.php', {
        ...
        .then(res => {
            ...
            if (res.status === 'success') {
                ...
                window.colorAreaActual = res.data.color_area || '#004D17';
...
function pintarDashboard(data) {
    ...
    if (tabActual === 'poda') {
        kpiArbolesBox.style.display = '';
        kpiArbolesBox.style.opacity = '1';
        kpiDineroBox.style.opacity = '1';
    } else if (tabActual === 'extras') {
        kpiArbolesBox.style.display = 'none';
        kpiDineroBox.style.opacity = '1';
    } else {
        // Mixto / General
        kpiArbolesBox.style.display = '';
...
function abrirDetallesPoda(index, desdeRanking = false) {
    ...
    // Actualizar encabezado según el tipo (en Mixto, decidimos según si tiene árboles)
...
function generarPDFPodaCorporativo(poda) {
    ...
    doc.text('Rancho Palmilla', PAGE_W / 2, y, { align: 'center' });
