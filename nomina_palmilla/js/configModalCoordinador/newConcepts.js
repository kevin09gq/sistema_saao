...
function mostrarEstructuraDeducciones() {
    ...
    `;
        ...
        $('#contenedor-deducciones-adicionales-coordinador').append(nuevaDeduccion);
...
function guardarDeduccionesExtra(empleado) {
    ...
    $('#contenedor-deducciones-adicionales-coordinador').find('.deduccion-extra-item').each(function () {
...
function guardarDescuentoOlvido(inputElement, olvidoObject, empleadoObject) {
    ...
    $('#mod-checador-coordinador').val(total.toFixed(2));
    establecerHistorialChecador(empleadoObject);
    calcularSueldoACobrar();
}
...
function guardarRetardoEditado(inputTolerancia, inputDescuentoMinuto, retardoObject, empleadoObject) {
    ...
    $('#mod-retardos-coordinador').val(total.toFixed(2));
    establecerHistorialRetardos(empleadoObject);
    calcularSueldoACobrar();
}
...
function guardarPermisoManual() {
    ...
    function recalcularDescuentoPermiso() {
        const minutos = parseInt($('#input-minutos-permiso-coordinador').val()) || 0;
        const costoMinuto = parseFloat($('#input-costo-minuto-permiso-coordinador').val()) || 0;
        const descuento = minutos * costoMinuto;
        $('#input-descuento-permiso-coordinador').val(descuento.toFixed(2));
    }

    // Agregar eventos de input para cálculo en tiempo real
    $('#input-minutos-permiso-coordinador').on('input', recalcularDescuentoPermiso);
    $('#input-costo-minuto-permiso-coordinador').on('input', recalcularDescuentoPermiso);
    ...
    $("#btn-agregar-permiso-coordinador").click(function (e) {
        ...
        // Obtener valores del formulario
        const dia = $('#select-dia-permiso-coordinador').val().trim();
        const minutos = parseInt($('#input-minutos-permiso-coordinador').val()) || 0;
        const costoMinuto = parseFloat($('#input-costo-minuto-permiso-coordinador').val()) || 0;
        const descuento = minutos * costoMinuto;
        ...
        // Limpiar formulario
        $('#select-dia-permiso-coordinador').val('');
        $('#input-minutos-permiso-coordinador').val('0');
        $('#input-costo-minuto-permiso-coordinador').val('0.00');
        $('#input-descuento-permiso-coordinador').val('0.00');
        ...
        $('#mod-permisos-coordinador').val(totalPermisos.toFixed(2));
...
function guardarUniformeManual() {
    $("#btn-agregar-uniforme-coordinador").click(function (e) {
        ...
        const cantidad = parseInt($('#input-cantidad-uniforme-coordinador').val()) || 0;
        if (!folio) {
        ...
        $('#input-folio-uniforme-coordinador').val('');
        $('#input-cantidad-uniforme-coordinador').val('0');
