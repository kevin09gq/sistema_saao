    ...
    $("#btn-export-jornalero-base").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                link.download = 'SEM ' + numeroSemana + ' - ' + aniosCierre + ' RANCHO LA PALMILLA NOMINAS - JORNALERO BASE - ' + timestamp + '.xlsx';
    ...
    $("#btn-export-jornalero-apoyo").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                var aniosCierre = jsonNominaPalmilla.fecha_cierre.split('/')[2];
                var timestamp = new Date().toISOString().replace(/T/, '_').replace(/:/g, '-').split('.')[0];
                link.download = 'SEM ' + numeroSemana + ' - ' + aniosCierre + ' RANCHO LA PALMILLA NOMINAS - JORNALERO APOYO - ' + timestamp + '.xlsx';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
       ...
       if (validarEmpleadosNegativos()) return;
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                var aniosCierre = jsonNominaPalmilla.fecha_cierre.split('/')[2];
                var timestamp = new Date().toISOString().replace(/T/, '_').replace(/:/g, '-').split('.')[0];
                link.download = 'SEM ' + numeroSemana + ' - ' + aniosCierre + ' RANCHO LA PALMILLA NOMINAS - COORDINADOR RANCHO - ' + timestamp + '.xlsx';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
    ...
    $("#btn-export-nomina-completa").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                var aniosCierre = jsonNominaPalmilla.fecha_cierre.split('/')[2];
                var timestamp = new Date().toISOString().replace(/T/, '_').replace(/:/g, '-').split('.')[0];
                link.download = 'SEM ' + numeroSemana + ' - ' + aniosCierre + ' RANCHO LA PALMILLA NOMINAS COMPLETAS - ' + timestamp + '.xlsx';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
    ...
    $("#btn-export-corte").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (blob) {
                ...
                var aniosCierre = jsonNominaPalmilla.fecha_cierre.split('/')[2];
                var timestamp = new Date().toISOString().replace(/T/, '_').replace(/:/g, '-').split('.')[0];
                link.download = 'SEM ' + numeroSemana + ' - ' + aniosCierre + ' RANCHO LA PALMILLA NOMINAS - CORTE REJAS DE LIMON - ' + timestamp + '.xlsx';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
