...
function guardarNominaHuasteca($data, $conexion)
{
    ...
    $query = "SELECT * FROM nomina_huasteca WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
    ...
    if ($result->num_rows > 0) {
        if ($actualizar) {
            // Actualizar nómina existente
             $updateQuery = "UPDATE nomina_huasteca SET nomina_huasteca = ?, total_percepciones = ?, total_deducciones = ?, total_neto = ? WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
        ...
        $insertQuery = "INSERT INTO nomina_huasteca (id_empresa, numero_semana, anio, nomina_huasteca, total_percepciones, total_deducciones, total_neto) VALUES (?, ?, ?, ?, ?, ?, ?)";
