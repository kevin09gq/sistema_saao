...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'GET':
        ...
        } else if (isset($_GET['case']) && $_GET['case'] === 'obtenerEmpleadosSinSeguroHuasteca') {
            obtenerEmpleadosSinSeguroHuasteca();
...
// PASO 1: OBTENER EMPLEADOS DE LOS DEPARTAMENTOS QUE ESTAN RELACIONADO A LA NOMINA HUASTECA (SIN SEGURO)
...
function obtenerEmpleadosSinSeguro()
{
    ...
    // Obtener empleados sin seguro que pertenecen al área y departamentos de la nómina 6
    $sql = "SELECT ie.id_empleado, ie.clave_empleado, ie.nombre, ie.ap_paterno, ie.ap_materno, ie.biometrico, ie.id_empresa, ie.id_departamento, ie.id_puestoEspecial
            ...
            AND ie.id_empresa = 1
            AND ie.id_area = (SELECT n.id_area FROM nombre_nominas n WHERE n.id_nomina = 6)
            AND ie.id_departamento IN (SELECT nd.id_departamento FROM nomina_departamento nd WHERE nd.id_nomina = 6)
            AND ie.status_nss = 0
...
function obtenerEmpleadosSinSeguroHuasteca()
{
    global $conexion;
...
function obtenerDepartamentosNomina()
{
    ...
    $idNomina = isset($_GET['id_nomina']) ? intval($_GET['id_nomina']) : 6;
