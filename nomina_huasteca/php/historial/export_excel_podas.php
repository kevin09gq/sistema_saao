...
function prepararHoja(
    ...
    $sheet->setCellValue('A2', 'RANCHO LA HUASTECA - HISTORIAL DE PODAS');
    ...
    // Títulos corporativos más grandes para impresión (Huasteca usa Verde)
    ...
    // Ancho de columnas equilibrado (Ligeramente reducido para asegurar impresión del Total)
    $anchos = [
...
try {
    ...
    $queryNominas = "
        SELECT DISTINCT n.id_nomina_huasteca, n.numero_semana, n.anio, n.nomina_huasteca
        FROM nomina_huasteca n
        JOIN podas_huasteca p ON p.id_nomina = n.id_nomina_huasteca
        WHERE {$where}
    ...
    while ($row = $resNom->fetch_assoc()) {
        $mesNom = extraerMesDesdeJson($row['nomina_huasteca']);
        if ($mesFiltroInt > 0 && (int)$mesNom !== $mesFiltroInt) {
            continue;
        }
        $nominaIds[] = (int)$row['id_nomina_huasteca'];
    ...
    $color_area = 'FF32BA5B'; // Verde por defecto en ARGB
    $query_color = "
        SELECT a.colores 
        FROM areas a
    ...
    $queryMov = "
        ...
        JOIN podas_huasteca p ON m.id_poda = p.id_poda
        JOIN nomina_huasteca n ON p.id_nomina = n.id_nomina_huasteca
        WHERE n.id_nomina_huasteca IN ({$placeholders})
    ...
    $spreadsheet->getProperties()
        ...
        ->setTitle('Historial Podas/Extras - Huasteca');
    ...
    $filename = "huasteca_{$anio}_{$semPart}_{$tipo}.xlsx";
