...
try {
    if ($action === 'get_filtros_iniciales') {
        $query = "
            ...
            JOIN cortes_huasteca c ON n.id_nomina_huasteca = c.id_nomina
    ...
    if ($action === 'buscar') {
        ...
        $query_cortes = "
            SELECT 
                ...
                n.numero_semana as sem_corte, n.anio as anio_corte, n.nomina_huasteca,
            ...
            FROM cortes_huasteca c
            JOIN nomina_huasteca n ON c.id_nomina = n.id_nomina_huasteca
            LEFT JOIN cortes_huasteca_tablas t ON c.id = t.id_corte
            WHERE $where
            GROUP BY c.id
        ...
        while ($c = $result_cortes->fetch_assoc()) {
            $mes_corte = extraerMesDesdeJson($c['nomina_huasteca']);
            
            if ($filtro_mes !== '' && (int)$mes_corte !== (int)$filtro_mes) {
                continue;
            }
            
            unset($c['nomina_huasteca']);
