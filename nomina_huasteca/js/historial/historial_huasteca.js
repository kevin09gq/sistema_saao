...
function cargarFiltrosIniciales() {
    ...
    fetch('../php/historial/get_historial_huasteca.php', {
        method: 'POST',
        body: formData
    })
        .then(r => r.json())
...
function exportarDesgloseTablaPDF() {
    ...
    const logoUrl = '../../public/img/huasteca.jpeg';
    const img = new Image();
...
function generarPDFConLogo(logoBase64, datosExportar, folioUnico = null) {
    ...
    doc.setTextColor(...COL_HEADER_TEXT);
    doc.text('Rancho Huasteca', PAGE_W / 2, 19, { align: 'center' });
    ...
    for (let pg = 1; pg <= totalPages; pg++) {
        ...
        doc.text(`Nómina Huasteca • Historial de Cortes • Página ${pg} de ${totalPages}`, PAGE_W / 2, PAGE_H - 10, { align: 'center' });
    }
    doc.save(folioUnico ? `Corte_Huasteca_${folioUnico}.pdf` : `Historial_Huasteca.pdf`);
}
