...
function llenar_cuerpo_tabla_pagos_por_dia() {
    ...
    const rangoFechas = obtenerRangoFechas(jsonNominaHuasteca.fecha_inicio, jsonNominaHuasteca.fecha_cierre);
...
function buscar_cortador() {
    $("#nombre_cortador").autocomplete({
        source: function (request, response) {
            ...
            let departamento = jsonNominaHuasteca.departamentos
...
function guardarTicketCorte(folio, nombreCortador, fecha, datosRejas, precio) {
    ...
    if (!departamento) {
        ...
        jsonNominaHuasteca.departamentos.push(departamento);
    }
...
function llenar_tabla_tickets_pendientes() {
    ...
    // VALIDAR QUE EXISTE EL jsonNominaHuasteca
    if (!jsonNominaHuasteca || !jsonNominaHuasteca.departamentos) {
        console.error("jsonNominaHuasteca no está definido o no tiene departamentos");
        return;
    }
    ...
    cortesPagina.forEach((corte, index) => {
        // Buscar si el folio ya está en la estructura jsonNominaHuasteca
        let existeFolio = false;

        // Buscar el departamento Corte
        let departamentoCorte = jsonNominaHuasteca.departamentos.find(
            d => d.nombre === "Corte"
        );
...
$(document).on("click", "#btn_copiar_salario", function (e) {
    ...
    if (jsonNominaHuasteca.departamentos) {
        corteDepto = jsonNominaHuasteca.departamentos.find(d => d.nombre === "Corte");
    }
...
$(document).on('submit', '#form_corte_nomina', function (e) {
    ...
    let departamento = jsonNominaHuasteca.departamentos.find(d => d.nombre === "Corte");
