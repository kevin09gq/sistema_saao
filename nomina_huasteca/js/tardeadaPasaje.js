...
function establecerDataModalTardeadaPasaje() {
    if (!jsonNominaHuasteca) {
        console.warn('jsonNominaHuasteca no está cargado aún');
        return;
    }
    $("#input-pasaje").val(jsonNominaHuasteca.precio_pasaje || 0);
    $("#input-comida").val(jsonNominaHuasteca.pago_comida || 0);
    $("#input-tardeada").val(jsonNominaHuasteca.pago_tardeada || 0);
}
...
function actualizarPasajeTardeadaEnEmpleados() {
    // Validar que exista jsonNominaHuasteca y horarioRancho
    if (!jsonNominaHuasteca || !jsonNominaHuasteca.horarioRancho) {
        console.warn('No se puede actualizar: falta jsonNominaHuasteca o horarioRancho');
        return;
    }

    // Recolectar empleados del departamento 13 que tengan registros
    const empleadosAActualizar = [];

    if (Array.isArray(jsonNominaHuasteca.departamentos)) {
        jsonNominaHuasteca.departamentos.forEach(departamento => {
            if (!Array.isArray(departamento.empleados)) return;

            departamento.empleados.forEach(empleado => {
                // Solo procesar departamento 13 con registros
                if (parseInt(empleado.id_departamento) === 13 && Array.isArray(empleado.registros)) {
...
function listarEmpleadosParaQuitarComidaPasaje() {
    if (!jsonNominaHuasteca || !Array.isArray(jsonNominaHuasteca.departamentos)) {
        console.warn('jsonNominaHuasteca no disponible');
        return;
    }
    ...
    // Iterar y filtrar jornaleros (departamento 13 con mostrar = true)
    jsonNominaHuasteca.departamentos.forEach(departamento => {
        if (!Array.isArray(departamento.empleados)) return;

        departamento.empleados.forEach(empleado => {
            if (parseInt(empleado.id_departamento) === 13 && empleado.mostrar !== false) {
                const fila = `
                    <tr>
