...
function crearEstructuraJson() {
    ...
    $('#btn_crear_nomina_huasteca').on('click', function () {
        ...
        // Cargar departamentos asociados a la nómina 6
        obtenerDepartamentosNomina(jsonNominaHuasteca);
    });
}

// PASO 2: Obtener los departamentos asociados a la nómina y agregarlos a la estructura del JSON
function obtenerDepartamentosNomina(jsonNominaHuasteca) {
    $.ajax({
            ...
            if (response.departamentos && response.departamentos.length > 0) {
                ...
                response.departamentos.forEach(function (dpto) {
                    jsonNominaHuasteca.departamentos.push({
                        ...
                        color_reporte: dpto.color_reporte || [], // El PHP ya envía el array de colores por empresa
                        empleados: []
                    });
                });
...
function obtenerEmpleadosSinSeguro(jsonNominaHuasteca) {
    $.ajax({
        url: '../php/validarExistenciaEmpleado.php',
        type: 'GET',
        data: {
            ...
            if (response.empleados && response.empleados.length > 0) {
                ...
                response.empleados.forEach(function (empleadoBD) {
                    ...
                    if (jsonNominaHuasteca) {
                        let dpto = jsonNominaHuasteca.departamentos.find(d => d.id_departamento == empleadoBD.id_departamento);
                        if (dpto) {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.existentes) {
                ...
                verificarEmpleadosSinSeguro(jsonNominaHuasteca);
...
function verificarEmpleadosSinSeguro(jsonNominaHuasteca) {
    ...
    // Obtener empleados sin seguro de la base de datos (Específico para Huasteca)
    $.ajax({
        url: '../php/validarExistenciaEmpleado.php',
        type: 'GET',
        data: {
            case: 'obtenerEmpleadosSinSeguroHuasteca'
        },
        dataType: 'json',
        success: function (response) {
            if (response.error) {
            ...
            if (response.empleados && response.empleados.length > 0) {
                ...
                empleadosSinSeguroNuevos.forEach(function (empSinSeguro) {
                    const deptoDestino = jsonNominaHuasteca.departamentos.find(d =>
