...
function guardarJustificacionGeneralCoordinadores() {
    ...
    // Validar que exista jsonNominaHuasteca 
    if (!jsonNominaHuasteca || !Array.isArray(jsonNominaHuasteca.departamentos)) {
        console.warn('jsonNominaHuasteca no está disponible');
        return;
    }
    ...
    jsonNominaHuasteca.departamentos.forEach(departamento => {
        ...
        departamento.empleados.forEach(empleado => {
            // Solo procesar coordinadores (id_departamento === 12)
            if (parseInt(empleado.id_departamento) !== 12) return;
