...
$(document).ready(function() {
    ...
    $('#btn_ticket_manual').on('click', function() {
        ...
        const nominaData = JSON.parse(localStorage.getItem('jsonNominaHuasteca') || '{}');
        // Actualizar variable global si hay datos más recientes
        if (nominaData && nominaData.departamentos) {
            if (typeof window.jsonNominaHuasteca !== 'undefined') {
                window.jsonNominaHuasteca = nominaData;
            }
        }
        if (!nominaData || !nominaData.departamentos) {
            Swal.fire({
    ...
    const listaHtml = `
        <div class="list-group">
            ${empleados.map(empleado => {
                ...
                // Departamentos específicos de Huasteca con colores únicos
...
function generarTicketsSeleccionados() {
    ...
    const nominaCompleta = JSON.parse(localStorage.getItem('jsonNominaHuasteca') || '{}');
    ...
    $.ajax({
        ...
        success: function(data, status, xhr) {
            ...
            a.href = url;
            a.download = `tickets_huasteca_seleccionados_${new Date().getTime()}.pdf`;
