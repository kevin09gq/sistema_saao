...
function confirmarsaveNomina() {
    $('#btn_guardar_nomina_huasteca').on('click', function () {
        Swal.fire({
            title: '¿Confirmar guardado?',
            text: `¿Está seguro que desea guardar la nómina de la semana ${jsonNominaHuasteca.numero_semana}?`,
            icon: 'question',
...
function saveNominaHuasteca() {
    ...
    const totales = calcularTotalesLocales(jsonNominaHuasteca);
    eliminarPropiedades(jsonNominaHuasteca); // Limpiar propiedades con valor 0 antes de guardar
    ...
    const departamentoPoda = jsonNominaHuasteca.departamentos.find(d => d.nombre === "Poda");
...
function getNominaHuasteca(numeroSemana, anio) {
    ...
    return $.ajax({
        ...
        try {
            ...
            console.error('getNominaHuasteca parse error', e, resp);
            return null;
        }
    }).catch(function (err) {
        console.error('getNominaHuasteca AJAX failed', err);
        return null;
    });
}
