...
foreach ($grupos as $nombreGrupo => $empleados) {
    ...
    foreach ($empleados as $idx => $emp) {
        ...
        if (($emp['uniformes'] ?? 0) != 0)
            $d['Uniforme'] = (float) $emp['uniformes'];
        if (($emp['checador'] ?? 0) != 0)
        ...
        // --- DESGLOSE DE HISTORIALES DE DEDUCCIONES ---
        $inasistenciaDetalle = [];
        if (!empty($emp['historial_inasistencias']) && is_array($emp['historial_inasistencias'])) {
            foreach ($emp['historial_inasistencias'] as $item) {
                $monto = (float) ($item['descuento_inasistencia'] ?? 0);
                if ($monto <= 0) continue;
                $dia = $item['dia'] ?? '';
                $min = (isset($item['minutos']) && (int)$item['minutos'] > 0) ? " (" . (int)$item['minutos'] . " min)" : "";
                $lbl = trim("{$dia}{$min}");
                if ($lbl === '') $lbl = 'Inasistencia';
                $inasistenciaDetalle[$lbl] = ($inasistenciaDetalle[$lbl] ?? 0) + $monto;
            }
        }

        $permisoDetalle = [];
        if (!empty($emp['historial_permisos']) && is_array($emp['historial_permisos'])) {
            foreach ($emp['historial_permisos'] as $item) {
                $monto = (float) ($item['descuento_permiso'] ?? 0);
                if ($monto <= 0) continue;
                $dia = $item['dia'] ?? '';
                $min = (isset($item['minutos_permiso']) && (int)$item['minutos_permiso'] > 0) ? " (" . (int)$item['minutos_permiso'] . " min)" : "";
                $lbl = trim("{$dia}{$min}");
                if ($lbl === '') $lbl = 'Permiso';
                $permisoDetalle[$lbl] = ($permisoDetalle[$lbl] ?? 0) + $monto;
            }
        }

        $uniformeDetalle = [];
        if (!empty($emp['historial_uniforme']) && is_array($emp['historial_uniforme'])) {
            foreach ($emp['historial_uniforme'] as $item) {
                $monto = (float) ($item['cantidad'] ?? 0);
                if ($monto <= 0) continue;
                $folio = trim($item['folio'] ?? '');
                $lbl = $folio !== '' ? "Folio: {$folio}" : 'Uniforme';
                $uniformeDetalle[$lbl] = ($uniformeDetalle[$lbl] ?? 0) + $monto;
            }
        }

        $biometricoDetalle = [];
        if (!empty($emp['historial_olvidos']) && is_array($emp['historial_olvidos'])) {
            foreach ($emp['historial_olvidos'] as $item) {
                $monto = (float) ($item['descuento_olvido'] ?? 0);
                if ($monto <= 0) continue;
                $lbl = $item['dia'] ?? $item['fecha'] ?? 'Olvido de Marcaje';
                $biometricoDetalle[$lbl] = ($biometricoDetalle[$lbl] ?? 0) + $monto;
            }
        }
        ...
        if (!empty($emp['historial_inasistencias']) && is_array($emp['historial_inasistencias'])) {
            ...
            $totDescuentoInas = 0;
            foreach ($emp['historial_inasistencias'] as $hInas) {
                $dia = !empty($hInas['dia']) ? $hInas['dia'] : '-';
        ...
        if (!empty($inasistenciaDetalle)) {
            foreach ($inasistenciaDetalle as $en => $ev) {
                $subK = "   • Inasistencia: {$en}";
        ...
        if (!empty($permisoDetalle)) {
            foreach ($permisoDetalle as $en => $ev) {
                $subK = "   • Permiso: {$en}";
                if (!isset($resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK]))
                    $resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK] = 0;
                $resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK] += $ev;
        ...
        if (!empty($uniformeDetalle)) {
            foreach ($uniformeDetalle as $en => $ev) {
                $subK = "   • Uniforme: {$en}";
                if (!isset($resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK]))
                    $resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK] = 0;
                $resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK] += $ev;
        ...
        if (!empty($biometricoDetalle)) {
            foreach ($biometricoDetalle as $en => $ev) {
                $subK = "   • Biométrico: {$en}";
                if (!isset($resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK]))
                    $resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK] = 0;
                $resumenPorTipo[$nombreGrupo]['deducciones_detalle'][$subK] += $ev;
