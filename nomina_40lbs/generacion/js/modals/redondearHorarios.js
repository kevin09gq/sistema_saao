    ...
    jsonNomina40lbs.departamentos.forEach(function (departamento) {
        ...
        departamento.empleados.forEach(function (empleado) {
            ...
            if (empleado.registros && empleado.registros.length > 0) {
                redondearRegistrosEmpleado(empleado, true);
            }
        });
    });
...
function redondearRegistrosEmpleado(empleado, editarHistoriales) {
    // Asegurar que el tabulador de sueldos esté cargado
    getTabulador();
    ...
    establecerSueldoNeto(empleado, editarHistoriales);
