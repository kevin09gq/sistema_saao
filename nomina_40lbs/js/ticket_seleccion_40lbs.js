...
function cargarEmpleadosParaTickets40lbs() {
    ...
    // Cargar todos los empleados de todos los departamentos
...
async function generarTicketsSeleccionados40lbs() {
    ...
    try {
        // Preparar estructura de datos para enviar al PHP - IGUAL QUE EN generarTicketsPDF()
        ...
        const nomina_para_enviar = {
            ...
            departamentos: jsonNomina40lbs.departamentos
                .map(depto => {
                    // Filtrar solo empleados seleccionados
                    const empleadosFiltrados = (depto.empleados || []).filter(emp => 
                        clavesSeleccionadas.includes(String(emp.clave || ''))
                    );
                    ...
                    return {
                        nombre: depto.nombre || '',
                        empleados: empleadosFiltrados.map(emp => {
