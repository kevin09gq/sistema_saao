...
function renderTicketPdf(TCPDF $pdf, $emp, $extra, $meta) {
    ...
    // Obtener departamento directamente de la BD en lugar del JSON
    $departamento = safeText($extra['nombre_departamento'] ?? '');
    $departamento = preg_replace('/^\d+/', '', $departamento);

    $rfc = safeText($extra['rfc_empleado'] ?? '');
    $imss = safeText($extra['imss'] ?? '');
    // Obtener puesto directamente de la BD en lugar del JSON
    ...
    if (toNumber($emp['redondeo'] ?? 0) > 0) $percepciones[] = ['label' => $conceptoNum++ . ' Redondeo', 'monto' => toNumber($emp['redondeo'])];
    foreach ($conceptosAdicionales as $c) {
    ...
    if (toNumber($emp['inasistencias_descuento'] ?? 0) > 0) $deducciones[] = ['label' => 'Ausentismo', 'monto' => toNumber($emp['inasistencias_descuento'])];
    ...
    $diferenciaRedondeo = $sueldoNetoRedondeado - $sueldoNeto;
    
    // Si la diferencia es positiva, agregar como percepción
    // Si es negativa, agregar como deducción (con valor absoluto)
    if (abs($diferenciaRedondeo) > 0.001) { // Evitar errores de precisión
        if ($diferenciaRedondeo > 0) {
            // Redondea hacia arriba - agregar como percepción
            $percepciones[] = ['label' => $conceptoNum++ . ' Redondeo', 'monto' => $diferenciaRedondeo];
        } else {
            // Redondea hacia abajo - agregar como deducción
            $deducciones[] = ['label' => 'Redondeo', 'monto' => abs($diferenciaRedondeo)];
        }
    }
    ...
    // Ajustar tamaño de fuente del puesto según longitud - SOLO SI TIENE PUESTO
    if (!empty($puesto)) {
        $puestoTexto = $puesto;
    ...
    // Calcular redondeo y agregarlo a percepciones o deducciones según corresponda
    $totalPerp = 0.0;
    foreach ($percepciones as $p) {
        $totalPerp += toNumber($p['monto']);
    }
    $totalDed = 0.0;
    foreach ($deducciones as $d) {
        $totalDed += toNumber($d['monto']);
    }
    ...
    if (abs($difRedondeo) > 0.001) {
        if ($difRedondeo > 0) {
            // Redondea hacia arriba - agregar como percepción
            $percepciones[] = ['label' => (count($percepciones) + 1) . ' Redondeo', 'monto' => $difRedondeo];
        } else {
            // Redondea hacia abajo - agregar como deducción
...
// Si la petición viene de selección manual, forzar modo "sin filtro" para incluir todos los empleados recibidos
if (isset($data['nomina']['departamentos']) && count($data['nomina']['departamentos']) > 0) {
    $totalEmpleados = 0;
    foreach ($data['nomina']['departamentos'] as $d) {
        if (isset($d['empleados']) && is_array($d['empleados'])) {
            $totalEmpleados += count($d['empleados']);
        }
    }
    // Si hay menos de 100 empleados, asumimos selección manual y forzamos sin filtro
    if ($totalEmpleados > 0 && $totalEmpleados < 100) {
...
foreach (($nomina['departamentos'] ?? []) as $depto) {
    ...
    if ($departamentoFiltrado !== null) {
        ...
        // Mapear el filtro a id_departamento y seguroSocial esperado
        $targetIdDepto = null;
        $targetSeguroSocial = null;

        if (stripos($filtroUpper, '40 LIBRAS') !== false) {
            $targetIdDepto = 4;
            $targetSeguroSocial = (stripos($filtroUpper, 'SSS') === false);
        } elseif (stripos($filtroUpper, '10 LIBRAS') !== false) {
            $targetIdDepto = 5;
            $targetSeguroSocial = (stripos($filtroUpper, 'SSS') === false);
        error_log("Filtro: $departamentoFiltrado | Target ID: $targetIdDepto | Target Seguro: " . ($targetSeguroSocial ? 'SI' : 'NO'));
    }

    foreach (($depto['empleados'] ?? []) as $emp) {
        if (is_array($emp)) {
            $empIdDepto = $emp['id_departamento'] ?? null;
            $empSeguroSocial = isset($emp['seguroSocial']) ? (bool)$emp['seguroSocial'] : true;

            // Si hay un filtro específico, el empleado debe coincidir en ID y Seguro Social
            if ($targetIdDepto !== null) {
                if ($empIdDepto == $targetIdDepto && $empSeguroSocial === $targetSeguroSocial) {
                    error_log("Empleado coincidente: " . ($emp['nombre'] ?? 'sin nombre') . " | ID Depto: $empIdDepto | Seguro: " . ($empSeguroSocial ? 'SI' : 'NO'));
                    $emp['departamento'] = $nombreDepto;
                    $empleados[] = $emp;
                }
            } else {
                // Si el filtro no es reconocido, incluir según la lógica anterior básica
                $empleados[] = $emp;
            }
        }
    }
    } else {
        // Si no hay filtro, incluir departamentos relevantes y filtrar empleados de 40/10
        if (stripos($nombreDeptoUpper, '40 LIBRAS') !== false || 
            ...
            foreach (($depto['empleados'] ?? []) as $emp) {
                if (is_array($emp)) {
                    $empIdDepto = $emp['id_departamento'] ?? null;
                    // Solo incluir si es departamento 4 (40lbs) o 5 (10lbs)
                    if ($empIdDepto == 4 || $empIdDepto == 5) {
                        $emp['departamento'] = $nombreDepto;
                        $empleados[] = $emp;
                    }
                }
...
// Usar clave+id_empresa para evitar confusión por claves repetidas
$clavesEmpresas = [];
foreach ($empleados as $e) {
    if (isset($e['clave']) && $e['clave'] !== '' && isset($e['id_empresa'])) {
        $clavesEmpresas[] = [
            'clave' => $e['clave'],
...
$extraMap = [];
if (count($clavesEmpresas) > 0) {
    $where = [];
    $params = [];
    $types = '';
    foreach ($clavesEmpresas as $ce) {
        $where[] = '(e.clave_empleado = ? AND e.id_empresa = ?)';
        $params[] = $ce['clave'];
...
foreach ($empleados as $idx => $emp) {
    ...
    $id_empresa = isset($emp['id_empresa']) ? $emp['id_empresa'] : '';
    $key = $clave . '_' . $id_empresa;
    $extra = ($clave && $id_empresa && isset($extraMap[$key])) ? $extraMap[$key] : [];
