...
$tmp_nombre = 'REPORTE_DISPERSION_TARJETA_PTU' . $anio . '_' . str_replace(' ', '_', $nombre_empresa) . '_' . date('Ymd_His');
...
function crearHoja(Spreadsheet $spreadsheet, String $titulo2, array $empleados, String $nombreHoja, String $color = '85C02A')
{
    ...
    // EMPRESAS
    $empresa_saao = 'CITRICOS SAAO';
    $empresa_sb = "SB CITRIC'S GROUP";
    ...
    $titulo3 = 'REPORTE DE DISPERSIÓN DE TARJETA (PTU) - ' . $anio;
    ...
    $sheet->getStyle('A2')->getFont()->setColor(new Color('000000')); // color negro
    ...
    $sheet->getStyle('A6:E6')->getFont()->setColor(new Color($color_letra));
    ...
    $sheet->getStyle('A6:E6')->getFill()->setFillType('solid');
    $sheet->getStyle('A6:E6')->getFill()->getStartColor()->setRGB($color);
    ...
    foreach ($empleados as $empleado) {
        ...
        $sheet->setCellValue('E' . $numeroFila, $empleado['tarjeta'] ?? 0);
        $sheet->getStyle('E' . $numeroFila)->getNumberFormat()->setFormatCode('[Red]-$#,##0.00;;');
    ...
    $sheet->getStyle('A' . $filaTotal . ':E' . $filaTotal)->getFill()
        ->setFillType('solid')->getStartColor()->setRGB('E8E8E8');
    // TAMAÑO DE LETRA EN LA FILA DE TOTALES
    $sheet->getStyle('A' . $filaTotal . ':E' . $filaTotal)->getFont()->setSize(16);
    // Centrar el texto en la fila de totales
    $sheet->getStyle('A' . $filaTotal . ':E' . $filaTotal)->getAlignment()->setVertical('center');
    $sheet->getStyle('D' . $filaTotal . ':E' . $filaTotal)->getAlignment()->setHorizontal('center');
    // Altura de la fila de totales
    $sheet->getRowDimension($filaTotal)->setRowHeight(46);

    // SUMAR VALORES DE LA COLUMNA E (DISPERSION TARJETA)
    $sheet->setCellValue('E' . $filaTotal, '=SUM(E' . $primera_fila . ':E' . $ultima_fila . ')');
    $sheet->getStyle('E' . $filaTotal)->getNumberFormat()->setFormatCode('[Red]-$#,##0.00;;');
    ...
    $sheet->getStyle('A6:E' . $filaTotal)->applyFromArray($estiloBordesTabla);
    ...
    $sheet->getPageSetup()->setPrintArea('A1:E' . $filaTotal);
}
...
foreach ($departamentos_seleccionados as $departamento) {
    ...
    $colorDepartamento = str_replace(
        '#',
        '',
        $empleados[0]['color_departamento'] ?? '85C02A'
    );
