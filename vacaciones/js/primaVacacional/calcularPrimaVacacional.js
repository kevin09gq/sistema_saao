...
$('#diasVacaciones, #salarioDiario, #porcentajePrima, #dispersionTarjeta, #isr').on('change input', function () {
    calcularPrima();
});
}
...
function calcularPrima() {
    ...
    // Fórmula: Días de vacaciones × Salario diario × (Porcentaje / 100)
    let montoPrima = (diasVacaciones * salarioDiario * (porcentajePrima / 100));
    ...
    $('#desglosesDias').text(diasVacaciones.toFixed(3));
...
function guardarPrima() {
    ...
    let diasVacaciones = $('#diasVacaciones').val();
    let domingos      = $('#domingos').val() || 0;
    ...
    // 2. Calcular montos
    let montoPrima  = (parseFloat(diasVacaciones) || 0) * (parseFloat(salarioDiario) || 0) * ((parseFloat(porcentajePrima) || 25) / 100);
