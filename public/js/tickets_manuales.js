...
// Función para abrir el modal de nómina relicario
function openNominaRelicarioModal() {
    activeModalType = 'relicario';
    
    // Cargar el CSS y JS específico de nómina relicario
    loadNominaRelicarioStylesAndScripts(function() {
        // Mostrar el modal de nómina relicario
        const modalElement = document.getElementById('modalTicketManualRelicario');
        if (modalElement) {
            const modal = new bootstrap.Modal(modalElement);
            modal.show();
            ...
            setTimeout(function() {
                // Inicializar autocompletado dentro del modal específico
                setupAutocompletadoRelicario(modalElement);
            }, 300);
        } else {
            alert('Error: No se encontró el modal de nómina relicario');
        }
    });
}
...
// Cargar estilos y scripts de nómina relicario
function loadNominaRelicarioStylesAndScripts(callback) {
    callback = callback || function() {};
    
    // Verificar si ya están cargados
    if (!window.nominaRelicarioTicketManualCssVersion) {
        window.nominaRelicarioTicketManualCssVersion = Date.now();
    }
    const cssHref = rutaRaiz + '/nomina_relicario/css/ticket_manual.css?v=' + window.nominaRelicarioTicketManualCssVersion;

    // Eliminar links viejos si existen
    document.querySelectorAll('link[href*="/nomina_relicario/css/ticket_manual.css"]').forEach(function(l) {
        if (l.getAttribute('data-ticket-manual') !== 'relicario') {
            l.parentNode && l.parentNode.removeChild(l);
        }
    });

    let link = document.querySelector('link[data-ticket-manual="relicario"]');
    if (!link) {
        link = document.createElement('link');
        link.rel = 'stylesheet';
        link.setAttribute('data-ticket-manual', 'relicario');
        document.head.appendChild(link);
    }
    link.href = cssHref;
    
    if (!window.nominaRelicarioTicketManualLoaded) {
        const script = document.createElement('script');
        script.src = rutaRaiz + '/nomina_relicario/js/ticket_manual.js';
        script.onload = function() {
            window.nominaRelicarioTicketManualLoaded = true;
            callback();
        };
        document.body.appendChild(script);
    } else {
...
// Función para inicializar autocompletado en el modal de nómina relicario
function setupAutocompletadoRelicario(modalElement) {
    if (!modalElement) return;
    ...
    form.addEventListener('submit', function(e) {
        e.preventDefault();
        generarTicketManualRelicario(form);
    });
    ...
    inputClave.addEventListener('input', function() {
        ...
        clearTimeout(timeoutBusqueda);
        timeoutBusqueda = setTimeout(() => {
            fetch(rutaRaiz + '/nomina_relicario/php/buscar_empleado.php?query=' + encodeURIComponent(query) + '&tipo=clave')
                .then(response => response.json())
                .then(empleados => {
                    suggestionsClave.innerHTML = '';
    ...
    inputNombre.addEventListener('input', function() {
        ...
        clearTimeout(timeoutBusqueda);
        timeoutBusqueda = setTimeout(() => {
            fetch(rutaRaiz + '/nomina_relicario/php/buscar_empleado.php?query=' + encodeURIComponent(query) + '&tipo=nombre')
                .then(response => response.json())
                .then(empleados => {
                    suggestionsNombre.innerHTML = '';
...
// Función para generar ticket manual de nómina relicario
function generarTicketManualRelicario(form) {
    const formData = new FormData(form);
    const data = {};
    ...
    // Asegurar que los campos numéricos tengan valor 0 si están vacíos
    const camposNumericos = [
        'salario_diario', 'salario_semanal', 'pasaje', 'comida', 'tardeada',
        'retardos', 'isr', 'imss_descuento', 'ajuste_sub', 'infonavit',
        'permiso', 'inasistencia', 'uniforme', 'checador', 'prestamo'
    ];
    ...
    // Enviar petición al servidor
    fetch(rutaRaiz + '/nomina_relicario/php/generar_ticket_manual.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    .then(blob => {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'ticket_manual_relicario_' + new Date().getTime() + '.pdf';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);
