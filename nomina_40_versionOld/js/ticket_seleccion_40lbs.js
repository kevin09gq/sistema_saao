...
$(document).ready(function() {
    ...
    $('#btn_ticket_manual_40lbs').on('click', function() {
        ...
        filtroDepartamentoActivo40lbs = 'todos';
        $('#filtro_departamento_ticket_40lbs').val('todos');
    ...
    $('#filtro_departamento_ticket_40lbs').on('change', function() {
        filtroDepartamentoActivo40lbs = $(this).val();
        filtrarEmpleados40lbs();
    });
    ...
    $('#btn_marcar_visibles_tickets_40lbs').on('click', function() {
        ...
        empleadosParaTickets40lbs.forEach(emp => {
            ...
            if (filtroDepartamentoActivo40lbs !== 'todos') {
                coincideDepto = (emp.departamento === filtroDepartamentoActivo40lbs);
            }
...
function cargarEmpleadosParaTickets40lbs() {
    ...
    const selectDepto = $('#filtro_departamento_ticket_40lbs');
    if (selectDepto.length) {
        const departamentosUnicos = [...new Set(empleadosParaTickets40lbs.map(emp => emp.departamento).filter(d => d))].sort();
        selectDepto.empty();
        ...
        if (departamentosUnicos.includes(filtroDepartamentoActivo40lbs)) {
            selectDepto.val(filtroDepartamentoActivo40lbs);
        } else {
            filtroDepartamentoActivo40lbs = 'todos';
...
let filtroDepartamentoActivo40lbs = 'todos';
...
async function generarTicketsSeleccionados40lbs() {
    try {
        ...
        const deptoCount = {};
        Object.keys(departamentosSeleccionados).forEach(deptoNombre => {
            deptoCount[deptoNombre.toUpperCase()] = departamentosSeleccionados[deptoNombre].length;
        });
        ...
        // Generar nombre del archivo con formato: SEMANA_DEPARTAMENTO_40LBS_AÑO (sin departamento si hay múltiples)
...
async function generarTicketsNombreSeleccionados40lbs() {
    try {
        ...
        // Generar nombre del archivo con formato: NOMBRE_SEMANA_DEPARTAMENTO_40LBS_AÑO (sin departamento si hay múltiples)
        const numSemana = nomina.numero_semana || '';
        const año = new Date().getFullYear();
        let nombreArchivo;
        if (multiplesDepartamentos) {
