...
function guardarNomina($data, $conexion) {
    ...
    $query = "SELECT * FROM nomina_40lbs WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
    ...
    if ($result->num_rows > 0) {
        if ($actualizar) {
            ...
            $updateQuery = "UPDATE nomina_40lbs SET nomina_40lbs = ?, total_percepciones = ?, total_deducciones = ?, total_neto = ? WHERE id_empresa = ? AND numero_semana = ? AND anio = ?";
        ...
        $insertQuery = "INSERT INTO nomina_40lbs (id_empresa, numero_semana, anio, nomina_40lbs, total_percepciones, total_deducciones, total_neto) VALUES (?, ?, ?, ?, ?, ?, ?)";
